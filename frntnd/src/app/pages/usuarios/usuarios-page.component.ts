  ...
  async cargarDatosAsync() {
    ...
    try {
      const [tipos, estados] = await Promise.all([
        toPromise(this.api.get<TipoUsuario[]>('tipos_usuario')),
        toPromise(this.api.get<Estado[]>('estados')),
      ]);
      ...
      const res: any = await toPromise(this.api.getPaginated('usuarios', { desde: 0, limite: 10 }));
      const rows = res?.data || [];
      this.data = rows.map((r: any) => ({
        id: r.id_usuario ?? r.id ?? r.ID ?? '',
        nombre: r.nombre_usuario ?? r.nombre ?? r.name ?? '',
