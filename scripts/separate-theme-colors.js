...
function isColorKey(key, value) {
  ...
  const colorKeyRe =
    /color|bg|accent|primary|secondary|header|breadcrumb|text|danger|success|info|warning|overlay|elev|btn|sidebar|breadcrumb/i;
...
async function main() {
  ...
  const [temas] = await db.query(
    "SELECT id_tema, clave, nombre, css_vars FROM adm_temas"
  );
  for (const t of temas) {
    ...
    if (newJson !== JSON.stringify(cssObj)) {
      console.log(
        `Actualizando tema ${t.id_tema} (${t.clave}) -> colores: ${
          Object.keys(newColors).length
        }`
      );
      await db.query("UPDATE adm_temas SET css_vars = ? WHERE id_tema = ?", [
        newJson,
        t.id_tema,
      ]);
    } else {
    ...
    for (const [key, val] of Object.entries(newColors)) {
      ...
      const [rows] = await db.query(
        "SELECT id_tema_var FROM adm_temas_variables WHERE clave = ? LIMIT 1",
        [key]
      );
      let id_tema_var;
      if (rows.length === 0) {
        ...
        const [res] = await db.query(
          "INSERT INTO adm_temas_variables (clave, etiqueta, tipo, valor_defecto) VALUES (?, ?, ?, ?) ",
          [key, etiqueta, "color", val]
        );
        ...
        await db.query(
          "UPDATE adm_temas_variables SET valor_defecto = COALESCE(valor_defecto, ?) WHERE id_tema_var = ?",
          [val, id_tema_var]
        );
      }
