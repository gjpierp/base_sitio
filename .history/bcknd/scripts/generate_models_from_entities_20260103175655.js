...
function generateModel(entity) {
  ...
  if (entity.columns.some(c => c.name === 'clave')) {
    ...
    if (hasAct) {
      lines.push("    const [rows] = await BaseModel.query('SELECT * FROM " + table + " WHERE clave = ? AND activo = 1', [clave]);");
    } else {
      lines.push("    const [rows] = await BaseModel.query('SELECT * FROM " + table + " WHERE clave = ?', [clave]);");
    }
  ...
  if (table === 'adm_usuarios') {
    ...
    lines.push("    const [rows] = await BaseModel.query('SELECT r.* FROM adm_roles r JOIN adm_usuarios_roles ur ON ur.id_rol = r.id_rol WHERE ur.id_usuario = ? AND ur.activo = 1 AND r.activo = 1', [id_usuario]);");
    lines.push("    return rows;");
    lines.push("  }");
    lines.push("");

    lines.push("  static async obtenerPermisos(id_usuario) {");
    lines.push("    const [rows] = await BaseModel.query('SELECT DISTINCT p.* FROM adm_permisos p JOIN adm_roles_permisos rp ON rp.id_permiso = p.id_permiso JOIN adm_usuarios_roles ur ON ur.id_rol = rp.id_rol WHERE ur.id_usuario = ? AND ur.activo = 1 AND rp.activo = 1 AND p.activo = 1', [id_usuario]);");
    lines.push("    return rows;");
    lines.push("  }");
    lines.push("");

    lines.push("  static async tieneRol(id_usuario, rolNombre) {");
    lines.push("    const [rows] = await BaseModel.query('SELECT 1 FROM adm_roles r JOIN adm_usuarios_roles ur ON ur.id_rol = r.id_rol WHERE ur.id_usuario = ? AND r.nombre = ? LIMIT 1', [id_usuario, rolNombre]);");
