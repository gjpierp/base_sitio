...
function generateModel(entity) {
  ...
  lines.push(
    "      const [rows] = await BaseModel.query('SELECT * FROM " + table + " ' + whereSql + ' ' + orderSql , vals);"
  );
  lines.push("      return rows;");
  lines.push("    }");
  lines.push(
    "    const [rows] = await BaseModel.query('SELECT * FROM " + table + " ' + whereSql + ' ' + orderSql + ' LIMIT ? OFFSET ?', vals.concat([limite, desde]));"
  );
  lines.push("    return rows;");
  lines.push("  }");
  lines.push("");
