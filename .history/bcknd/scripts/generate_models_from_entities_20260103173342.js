...
function detectIdColumn(entity) {
  if (entity.primaryKey && entity.primaryKey.length) return entity.primaryKey[0];
...
function relevantSearchFields(entity) {
  ...
  const prefer = ['nombre', 'codigo', 'correo', 'correo_electronico', 'email', 'dominio', 'url', 'host'];
...
function generateModel(entity) {
  ...
  lines.push(`    const [rows] = await db.query('SELECT * FROM ${table} WHERE ${idcol} = ?', [${idcol}]);`);
  ...
  lines.push(`    const [rows] = await db.query('SELECT * FROM ${table} ${hasAct ? "WHERE activo = 1" : ""} ORDER BY 1 DESC');`);
  lines.push('    return rows;');
  lines.push('  }');
  lines.push('');

  // listar (paginación)
  lines.push('  static async listar(desde = 0, limite = 10, sortColumn = null, sortDir = "ASC", filters = {}) {');
  ...
  lines.push('    const whereSql = whereParts.length ? "WHERE " + whereParts.join(" AND ") : "";');
  lines.push('    const dir = (String(sortDir || "ASC").toUpperCase() === "DESC") ? "DESC" : "ASC";');
  lines.push('    const orderSql = sortColumn ? `ORDER BY ${sortColumn} ${dir}` : "";');
  lines.push('    if (typeof desde === "undefined" || typeof limite === "undefined") {');
  lines.push('      const [rows] = await db.query(`SELECT * FROM ${table} ${whereSql} ${orderSql}` , vals);');
  lines.push('      return rows;');
  lines.push('    }');
  lines.push('    const [rows] = await db.query(`SELECT * FROM ${table} ${whereSql} ${orderSql} LIMIT ? OFFSET ?`, vals.concat([limite, desde]));');
  lines.push('    return rows;');
  lines.push('  }');
  lines.push('');
  ...
  lines.push('    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";');
  lines.push('    const [rows] = await db.query(`SELECT COUNT(*) as total FROM ${table} ${whereSql}`, vals);');
  ...
  lines.push('    const keys = Object.keys(data || {}).filter(k=>data[k] !== undefined);');
  lines.push('    const vals = keys.map(k=>data[k]);');
  lines.push('    const sql = `INSERT INTO ${table} (${keys.join(",")}) VALUES (${keys.map(_=>"?").join(",")})`;');
  ...
  lines.push('    const vals = keys.map(k=>data[k]);');
  lines.push("    await db.query('UPDATE ${table} SET ' + setSql + ' WHERE ${idcol} = ?', vals.concat([${idcol}]));");
  lines.push('  }');
  lines.push('');

  // delete
  lines.push(`  static async delete(${idcol}) {`);
  lines.push(`    await db.query('DELETE FROM ${table} WHERE ${idcol} = ?', [${idcol}]);`);
  lines.push('  }');
  lines.push('');
  ...
  if (hasAct) {
    ...
    lines.push(`    await db.query('UPDATE ${table} SET activo = 0 WHERE ${idcol} = ?', [${idcol}]);`);
    lines.push('  }');
    lines.push('');
  }
  ...
  lines.push('    const where = [];');
  lines.push('    const vals = [];');
  lines.push('    for (const k of Object.keys(filters || {})) { if (filters[k] === undefined || filters[k] === null) continue; where.push(`${k} = ?`); vals.push(filters[k]); }');
  lines.push('    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";');
  lines.push('    const [rows] = await db.query(`SELECT * FROM ${table} ${whereSql}`, vals);');
  lines.push('    return rows;');
  lines.push('  }');
...
async function main() {
  const entitiesDir = path.resolve(__dirname, '..', 'entities', 'db_vvff_admin');
  ...
  for (const f of files) {
    ...
    let entity;
    try { entity = JSON.parse(content); } catch (e) { console.warn('skip', f); continue; }
