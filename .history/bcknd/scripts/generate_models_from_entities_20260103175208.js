...
function generateModel(entity) {
  ...
  lines.push(
    "    const [rows] = await BaseModel.query('SELECT COUNT(*) as total FROM ' + table + ' ' + whereSql, vals);"
  );
  ...
  lines.push("    const vals = keys.map(k=>data[k]);");
  lines.push(
    "    await BaseModel.query('UPDATE ' + table + ' SET ' + setSql + ' WHERE " +
      idcol +
      " = ?', vals.concat([" +
      idcol +
      "]));"
  ...
  lines.push(
    "    await BaseModel.query('DELETE FROM ' + table + ' WHERE " +
      idcol +
      " = ?', [" +
      idcol +
      "]); "
  ...
  if (hasAct) {
    ...
    lines.push(
      "    await BaseModel.query('UPDATE ' + table + ' SET activo = 0 WHERE " +
        idcol +
        " = ?', [" +
        idcol +
        "]); "
  ...
  lines.push(
    "    const [rows] = await BaseModel.query('SELECT * FROM ' + table + ' ' + whereSql, vals);"
  );
  lines.push("    return rows;");
  lines.push("  }");
