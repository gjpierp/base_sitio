...
const getUsuariosPaginados = async (req, res = response) => {
  try {
    ...
    const safe = Array.isArray(usuarios)
      ...
      : usuarios;
    res.json({ ok: true, usuarios: safe, total, desde, limite, page, per_page });
  } catch (error) {
    console.error(error);
    res.status(500).json({
      ok: false,
  ...
  try {
    ...
    switch (tabla) {
      ...
      case "jerarquias": {
        ...
        const jerarquias = await AdmJerarquia.findAll();
        data = jerarquias.filter(
          (j) =>
            (j.nombre && j.nombre.match(regex)) ||
            (j.descripcion && j.descripcion.match(regex))
        );
        break;
      }
      default:
    ...
    res.json({ ok: true, resultados, meta: { total, desde, limite, page, per_page } });
  } catch (error) {
    console.error(error);
    res.status(500).json({
      ok: false,
