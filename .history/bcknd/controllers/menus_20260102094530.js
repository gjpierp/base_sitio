...
const getMenusPaginados = async (req, res = response) => {
  try {
    ...
    const perPage = !isNaN(perPageParam) && perPageParam > 0 ? perPageParam : defaultItemsPerPage;
    if (desde === undefined && hasta === undefined && isNaN(limiteParam) && isNaN(pageParam)) {
...
const obtenerMenus = async (req, res = response) => {
  try {
    ...
    if (!isNaN(pageParam) && pageParam >= 1) {
      ...
      const total = await Menu.contar();
      return res.json({ ok: true, menus, total, page, per_page: perPage, desde, limite });
    }
