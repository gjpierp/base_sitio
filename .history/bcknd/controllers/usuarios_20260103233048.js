...
const actualizarUsuario = async (req, res = response) => {
  ...
  const camposRoles = ["roles", "permisos", "id_rol", "id_permisos", "id_jerarquia", "id_jerarquias"];
  const camposEnRequest = Object.keys(req.body || {});
  const modificaRolesOPermisos = camposEnRequest.some((k) => camposRoles.includes(k));
  let esAdmin = false;
  if (modificaRolesOPermisos) {
    // Verificar si el usuario autenticado es admin
    esAdmin = await Usuario.tieneRol(req.uid, "ADMIN_ROLE");
    if (!esAdmin) {
  ...
  try {
    ...
    // Construir objeto de actualización solo con campos enviados por el cliente
    const datosActualizacion = { ...campos };
    // Capturar payload de roles (si viene) para sincronizar vía tabla relacional
    let rolesPayload = null;
    if (typeof roles !== 'undefined') rolesPayload = Array.isArray(roles) ? roles : [roles];
    else if (typeof id_rol !== 'undefined') rolesPayload = Array.isArray(id_rol) ? id_rol : [id_rol];
    // No permitir cambiar el correo aquí; usar el valor actual de la DB
    datosActualizacion.correo = usuario.correo;
    // Eliminar alias y campos no mapeados a columnas reales
    delete datosActualizacion.correo_electronico;
    delete datosActualizacion.email;
    // Eliminar cualquier campo relacionado con roles/jerarquias para evitar incluirlos en el UPDATE
    delete datosActualizacion.roles;
    delete datosActualizacion.permisos;
    delete datosActualizacion.id_rol;
    delete datosActualizacion.id_permisos;
    delete datosActualizacion.id_jerarquia;
    delete datosActualizacion.id_jerarquias;
    // Evitar actualizar PKs o timestamps por accidente
    delete datosActualizacion.id_usuario;
    delete datosActualizacion.fecha_creacion;
    delete datosActualizacion.fecha_actualizacion;
    ...
    // Si se recibió un payload de roles y el actor es admin, sincronizar relaciones en tabla puente
    try {
      if (rolesPayload && esAdmin) {
        await Usuario.sincronizarRoles(uid, rolesPayload);
      }
    } catch (e) {
      console.error('[actualizarUsuario] error sincronizando roles', e);
    }
...
const cambiarContrasena = async (req, res = response) => {
  ...
  try {
    ...
    // Buscar intentos fallidos en el intervalo usando la columna real `fecha`
    const [intentosRows] = await BaseModel.query(
      `SELECT * FROM adm_historial_acciones WHERE id_usuario = ? AND accion = ? AND fecha >= ? LIMIT ? OFFSET ?`,
      [uid, "CAMBIO_CONTRASENA_FALLIDO", desdeStr, 100, 0]
    );
