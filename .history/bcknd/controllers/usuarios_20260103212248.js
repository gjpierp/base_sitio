...
const crearUsuario = async (req, res = response) => {
  ...
  const body = req.body || {};
  const correo_electronico = body.correo_electronico || body.correo || body.email;
  ...
  let id_tipo_usuario =
    body.id_tipo_usuario || body.tipoUsuario || body.idTipoUsuario || body.id_tipoUsuario || body.tipo_usuario;

  // Normalize numeric strings to numbers when possible
  if (typeof id_rol === 'string' && id_rol.trim() !== '') id_rol = Number(id_rol);
  if (typeof id_jerarquia === 'string' && id_jerarquia.trim() !== '') id_jerarquia = Number(id_jerarquia);
  if (typeof id_tipo_usuario === 'string' && id_tipo_usuario.trim() !== '') id_tipo_usuario = Number(id_tipo_usuario);
  ...
  try {
    ...
    // Guardar usuario
    const insertId = await Usuario.crear(usuarioData);
