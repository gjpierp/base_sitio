...
const login = async (req, res = response) => {
  ...
  try {
    const usuarioDB = await AdmUsuario.findAll();
    const usuario = usuarioDB.find((u) => u.correo === correo);
    if (!usuario) {
    ...
    // Obtener roles
    const rolesRel = await AdmUsuarioRol.findAll();
    const rolesIds = rolesRel
...
const googleSignIn = async (req, res = response) => {
  ...
  try {
    ...
    let usuarioDB = (await AdmUsuario.findAll()).find(
      (u) => u.correo === email
    );
    let usuarioId;
    let usuarioData;
    ...
    const rolesRel = await AdmUsuarioRol.findAll();
    const rolesIds = rolesRel
      .filter((r) => r.id_usuario === usuarioId)
      .map((r) => r.id_rol);
    const roles = (await AdmRol.findAll()).filter((r) =>
      rolesIds.includes(r.id_rol)
    );
