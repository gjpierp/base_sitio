...
const obtenerMenusUsuario = async (req, res = response) => {
  ...
  try {
    ...
    if (!Array.isArray(permisos)) return res.status(400).json({ ok: false, msg: 'El campo permisos debe ser un array de ids' });

    // Normalizar ids
    const permisosIds = permisos.map(p => Number(p)).filter(n => !isNaN(n));
    if (permisosIds.length === 0) return res.status(400).json({ ok: false, msg: 'No se enviaron ids de permisos válidos' });

    // Verificar existencia de permisos en la tabla adm_permisos
    const dbPool = require('../database/config');
    const [foundPerms] = await dbPool.query(`SELECT id_permiso FROM adm_permisos WHERE id_permiso IN (${permisosIds.map(()=>'?').join(',')})`, permisosIds);
    const existingIds = (foundPerms || []).map(r => Number(r.id_permiso));
    const invalid = permisosIds.filter(p => !existingIds.includes(p));
    if (invalid.length) return res.status(400).json({ ok: false, msg: 'Algunos permisos no existen', invalid });
