...
const getMenusPaginados = async (req, res = response) => {
  try {
    ...
    const defaultItemsPerPage =
      cfg && !isNaN(Number(cfg.valor)) ? Number(cfg.valor) : 10;
    // Soportar page/per_page (page 1-based)
    const pageParam = Number(req.query.page);
    const perPageParam = Number(req.query.per_page ?? req.query.perPage);
    const perPage =
      !isNaN(perPageParam) && perPageParam > 0
        ? perPageParam
        : defaultItemsPerPage;
    if (
      desde === undefined &&
      ...
      // Si no se pasan parámetros ni page, usar paginación global: primera página
      desde = 0;
      hasta = defaultItemsPerPage;
    }

    // Si se pasa page, calcular offset y límite desde page/per_page
    if (!isNaN(pageParam) && pageParam >= 1) {
      const page = Math.floor(pageParam);
      desde = (page - 1) * perPage;
      hasta = desde + perPage;
      // Forzar limite a perPage (se sobrescribirá más abajo si es necesario)
      req.query.limite = String(perPage);
    }
    desde = Number(desde);
    hasta = Number(hasta);
...
const obtenerMenus = async (req, res = response) => {
  try {
    // Soportar page/per_page para paginar resultados
    const pageParam = Number(req.query.page);
    const perPageParam = Number(req.query.per_page ?? req.query.perPage);
    const cfg = await AdmConfiguraciones.findByClave("items_per_page");
    const defaultItemsPerPage =
    ...
    if (!isNaN(pageParam) && pageParam >= 1) {
      ...
      const limite = perPage;
      const menus = await Menu.listar(desde, limite);
      const total = await Menu.contar();
      return res.json({
        ok: true,
        menus,
    ...
    // Si no se pasa page, conservar comportamiento previo (devolver todos)
...
const crearMenu = async (req, res = response) => {
  try {
    ...
    const existeNombre = await Menu.existsByNombre(datos.nombre);
    if (existeNombre) {
      return res
        .status(400)
    ...
    // Validar url única
    const existeUrl = await Menu.existsByUrl(datos.url);
    if (existeUrl) {
      return res
        .status(400)
...
const actualizarMenu = async (req, res = response) => {
  try {
    ...
    if (datos.nombre && datos.nombre !== menu.nombre) {
      const existeNombre = await Menu.existsByNombre(datos.nombre, id);
      if (existeNombre) {
        return res
          .status(400)
          .json({ ok: false, msg: "Ya existe un menú con ese nombre" });
    ...
    if (datos.url && datos.url !== menu.url) {
      const existeUrl = await Menu.existsByUrl(datos.url, id);
      if (existeUrl) {
        return res
          .status(400)
          .json({ ok: false, msg: "Ya existe un menú con esa url" });
