...
const login = async (req, res = response) => {
  ...
  try {
    ...
    if (!usuario) {
      ...
      // No insertamos id_usuario = 0 porque la columna tiene FK a adm_usuarios.
      // Omitimos id_usuario para evitar violación de FK; si la columna permite NULL,
      // quedará registrado sin referencia.
...
const registro = async (req, res = response) => {
  ...
  try {
    ...
    const usuarioExistente = await AdmUsuario.obtenerPorCorreo(correo);
    if (usuarioExistente) {
      return res
        .status(400)
