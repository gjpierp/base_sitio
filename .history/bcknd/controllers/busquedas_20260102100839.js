...
const getTodo = async (req, res = response) => {
  try {
    ...
    if (safe.length === 0) {
      return res.status(404).json({ ok: false, msg: "No se encontraron usuarios con ese criterio" });
    }
  ...
  try {
    ...
    switch (tabla) {
      ...
      case "roles": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM adm_roles WHERE LOWER(nombre) LIKE ? OR LOWER(descripcion) LIKE ?`, [qLike, qLike]);
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(`SELECT * FROM adm_roles WHERE LOWER(nombre) LIKE ? OR LOWER(descripcion) LIKE ? ORDER BY nombre LIMIT ? OFFSET ?`, [qLike, qLike, limite, desde]);
        data = rows;
        data._total = total;
        break;
      }
      case "permisos": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM adm_permisos WHERE LOWER(nombre) LIKE ? OR LOWER(descripcion) LIKE ?`, [qLike, qLike]);
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(`SELECT * FROM adm_permisos WHERE LOWER(nombre) LIKE ? OR LOWER(descripcion) LIKE ? ORDER BY nombre LIMIT ? OFFSET ?`, [qLike, qLike, limite, desde]);
        data = rows;
        data._total = total;
        break;
      }
      case "menus": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM adm_menus WHERE LOWER(nombre) LIKE ? OR LOWER(url) LIKE ? OR LOWER(icono) LIKE ?`, [qLike, qLike, qLike]);
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(`SELECT * FROM adm_menus WHERE LOWER(nombre) LIKE ? OR LOWER(url) LIKE ? OR LOWER(icono) LIKE ? ORDER BY nombre LIMIT ? OFFSET ?`, [qLike, qLike, qLike, limite, desde]);
        data = rows;
        data._total = total;
        break;
      }
      case "aplicaciones_sitio": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM adm_aplicaciones_sitio WHERE LOWER(nombre) LIKE ? OR LOWER(clave) LIKE ? OR LOWER(descripcion) LIKE ?`, [qLike, qLike, qLike]);
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(`SELECT * FROM adm_aplicaciones_sitio WHERE LOWER(nombre) LIKE ? OR LOWER(clave) LIKE ? OR LOWER(descripcion) LIKE ? ORDER BY nombre LIMIT ? OFFSET ?`, [qLike, qLike, qLike, limite, desde]);
        data = rows;
        data._total = total;
        break;
      }
      case "sitios": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM adm_sitios WHERE LOWER(nombre) LIKE ? OR LOWER(codigo) LIKE ? OR LOWER(dominio) LIKE ?`, [qLike, qLike, qLike]);
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(`SELECT * FROM adm_sitios WHERE LOWER(nombre) LIKE ? OR LOWER(codigo) LIKE ? OR LOWER(dominio) LIKE ? ORDER BY nombre LIMIT ? OFFSET ?`, [qLike, qLike, qLike, limite, desde]);
        data = rows;
        data._total = total;
        break;
      }
      case "configuraciones": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM adm_configuraciones WHERE LOWER(clave) LIKE ? OR LOWER(COALESCE(valor, '')) LIKE ? OR LOWER(COALESCE(descripcion, '')) LIKE ?`, [qLike, qLike, qLike]);
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(`SELECT * FROM adm_configuraciones WHERE LOWER(clave) LIKE ? OR LOWER(COALESCE(valor, '')) LIKE ? OR LOWER(COALESCE(descripcion, '')) LIKE ? ORDER BY clave LIMIT ? OFFSET ?`, [qLike, qLike, qLike, limite, desde]);
        data = rows;
        data._total = total;
        break;
      }
      case "tipos_usuario": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM adm_tipos_usuario WHERE LOWER(nombre) LIKE ? OR LOWER(COALESCE(descripcion,'')) LIKE ?`, [qLike, qLike]);
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(`SELECT * FROM adm_tipos_usuario WHERE LOWER(nombre) LIKE ? OR LOWER(COALESCE(descripcion,'')) LIKE ? ORDER BY nombre LIMIT ? OFFSET ?`, [qLike, qLike, limite, desde]);
        data = rows;
        data._total = total;
        break;
      }
      case "estados": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM adm_estados WHERE LOWER(nombre) LIKE ? OR LOWER(COALESCE(descripcion,'')) LIKE ?`, [qLike, qLike]);
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(`SELECT * FROM adm_estados WHERE LOWER(nombre) LIKE ? OR LOWER(COALESCE(descripcion,'')) LIKE ? ORDER BY nombre LIMIT ? OFFSET ?`, [qLike, qLike, limite, desde]);
        data = rows;
        data._total = total;
        break;
      }
      case "jerarquias": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM adm_jerarquias WHERE LOWER(nombre) LIKE ? OR LOWER(COALESCE(descripcion,'')) LIKE ?`, [qLike, qLike]);
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(`SELECT * FROM adm_jerarquias WHERE LOWER(nombre) LIKE ? OR LOWER(COALESCE(descripcion,'')) LIKE ? ORDER BY nombre LIMIT ? OFFSET ?`, [qLike, qLike, limite, desde]);
        data = rows;
        data._total = total;
        break;
      }
    ...
    const total = typeof data._total === "number" ? data._total : Array.isArray(data) ? data.length : 0;
