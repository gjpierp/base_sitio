  ...
  try {
    ...
    if (id_usuario === null) {
      const [countRows] = await db.query(
        `SELECT COUNT(*) as total FROM adm_configuraciones`
      );
      total = countRows[0]?.total || 0;
      const [dataRows] = await db.query(
        `SELECT * FROM adm_configuraciones ORDER BY clave LIMIT ? OFFSET ?`,
        [limite, desde]
      );
      rows = dataRows;
    } else {
      const [countRows] = await db.query(
        `SELECT COUNT(*) as total FROM adm_configuraciones WHERE id_usuario = ? OR id_usuario IS NULL`,
        [id_usuario]
      );
      total = countRows[0]?.total || 0;
      const [dataRows] = await db.query(
        `SELECT * FROM adm_configuraciones WHERE id_usuario = ? OR id_usuario IS NULL ORDER BY id_usuario DESC, clave LIMIT ? OFFSET ?`,
        [id_usuario, limite, desde]
      );
      rows = dataRows;
    }
    res.json({ ok: true, data: rows, meta: { total, desde, limite, page, per_page } });
  } catch (error) {
...
const getConfiguracion = async (req, res = response) => {
  try {
    ...
    if (!config) {
      return res
        .status(404)
        .json({ ok: false, msg: "Configuración no encontrada" });
    }
...
const listUserConfigs = async (req, res = response) => {
  try {
    ...
    const { desde, limite, page, per_page } = await parsePagination(req);
    const [countRows] = await db.query(
      `SELECT COUNT(*) as total FROM adm_configuraciones WHERE id_usuario = ?`,
      [uid]
    );
    const total = countRows[0]?.total || 0;
    const [rows] = await db.query(
      `SELECT * FROM adm_configuraciones WHERE id_usuario = ? ORDER BY clave LIMIT ? OFFSET ?`,
      [uid, limite, desde]
    );
    res.json({ ok: true, data: rows, meta: { total, desde, limite, page, per_page } });
  } catch (error) {
