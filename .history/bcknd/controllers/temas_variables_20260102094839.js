...
exports.listVariables = async (req, res, next) => {
  try {
    ...
    if (q) {
      ...
      vars = vars.filter(
        (v) =>
          (v.clave || "").toLowerCase().includes(ql) ||
          (v.etiqueta || "").toLowerCase().includes(ql)
      );
    }
    const { desde, limite, page, per_page } = await parsePagination(req);
    const total = Array.isArray(vars) ? vars.length : 0;
    const data = Array.isArray(vars) ? vars.slice(desde, desde + limite) : vars;
    res.json({ ok: true, variables: data, total, page, per_page, desde, limite });
  } catch (err) {
    next(err);
  }
};
