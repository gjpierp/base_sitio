 ...
 *
 * Exporta:
 * - obtenerMenus, obtenerArbolMenus, obtenerMenu, crearMenu, actualizarMenu,
 *   eliminarMenu, obtenerMenusUsuario, asignarPermisos, obtenerPermisosMenu,
 *   getMenusPaginados
 */
...
const asignarPermisos = async (req, res = response) => {
  ...
  try {
    ...
    if (!Array.isArray(permisos)) {
      return res.status(400).json({ ok: false, msg: 'El campo permisos debe ser un array de ids' });
    }

    // Normalizar ids a números y filtrar inválidos
    ...
    if (replace) {
      await conn.query('DELETE FROM adm_menus_permisos WHERE id_menu = ?', [id]);
    }

    const created = [];
    for (const id_permiso of permisosIds) {
      const [rows] = await conn.query('SELECT 1 FROM adm_menus_permisos WHERE id_menu = ? AND id_permiso = ? LIMIT 1', [id, id_permiso]);
      const exists = Array.isArray(rows) ? rows.length > 0 : rows && rows.length > 0;
      if (!exists) {
        await conn.query('INSERT INTO adm_menus_permisos (id_menu, id_permiso, activo) VALUES (?, ?, ?)', [id, id_permiso, 1]);
        created.push(id_permiso);
      }
    }

    await conn.commit();
    res.json({ ok: true, msg: 'Permisos asignados/actualizados', created, total: created.length });
  } catch (error) {
    console.error(error);
    if (conn) {
      try {
    ...
    res.status(500).json({
      ok: false,
      msg: "Error al obtener menús paginados",
    });
  }
};
...
const obtenerMenusUsuario = async (req, res = response) => {
  try {
    ...
    const arbol = await Menu.buildUserMenuTree(idUsuario);
    res.json({
      ok: true,
      menus: arbol,
    });
    ...
    res.status(500).json({
      ok: false,
      msg: "Error al obtener menús del usuario",
    });
  }
};
...
const asignarPermisos = async (req, res = response) => {
  try {
    ...
    // Si body.replace === true, reemplazar relaciones existentes
    const replace = req.body.replace === true || req.body.replace === "true";
    // Insertar permisions faltantes (o reemplazar todo)
    if (replace) {
      await AdmMenuPermiso.deleteByMenuId(id);
    }
    const created = [];
    for (const id_permiso of permisos) {
      const yaAsignado = await AdmMenuPermiso.existeRelacion(id, id_permiso);
      if (!yaAsignado) {
        await AdmMenuPermiso.create({ id_menu: id, id_permiso });
        created.push(id_permiso);
      }
    }
    res.json({
...
const eliminarPermisoMenu = async (req, res = response) => {
  try {
    ...
    const menu = await Menu.findById(id);
    if (!menu) return res.status(404).json({ ok: false, msg: 'Menú no encontrado' });
    const AdmMenuPermiso = require('../models/adm_menus_permisos');
    // comprobar existencia de permiso
    const [found] = await require('../database/config').query('SELECT 1 FROM adm_permisos WHERE id_permiso = ? LIMIT 1', [permId]);
    const existsPerm = Array.isArray(found) ? found.length > 0 : !!found;
    if (!existsPerm) return res.status(404).json({ ok: false, msg: 'Permiso no encontrado' });
    await AdmMenuPermiso.deleteByMenuAndPerm(id, permId);
    res.json({ ok: true, msg: 'Permiso eliminado del menú' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ ok: false, msg: 'Error al eliminar permiso del menú' });
  }
};
