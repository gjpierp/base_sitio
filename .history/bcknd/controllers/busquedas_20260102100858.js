  ...
  try {
    ...
    switch (tabla) {
      ...
      case "sitios": {
        ...
        const qLike = `%${busqueda.toLowerCase()}%`;
        const [countRows] = await db.query(
          `SELECT COUNT(*) as total FROM adm_sitios WHERE LOWER(nombre) LIKE ? OR LOWER(codigo) LIKE ? OR LOWER(dominio) LIKE ?`,
          [qLike, qLike, qLike]
        );
        const total = countRows[0]?.total || 0;
        const [rows] = await db.query(
          `SELECT * FROM adm_sitios WHERE LOWER(nombre) LIKE ? OR LOWER(codigo) LIKE ? OR LOWER(dominio) LIKE ? ORDER BY nombre LIMIT ? OFFSET ?`,
          [qLike, qLike, qLike, limite, desde]
        );
        data = rows;
        data._total = total;
