...
exports.obtenerUsuariosJerarquias = async (req, res, next) => {
  try {
    ...
    // Permitir filtros via query (ej. ?id_usuario=123)
    const filters = {};
    if (req.query && typeof req.query.id_usuario !== 'undefined') {
      filters.id_usuario = req.query.id_usuario;
    }
    const lista = await AdmUsuarioJerarquia.listar(desde, limite, null, 'ASC', filters);
    const total = await AdmUsuarioJerarquia.contar(filters);
    res.json({
      ok: true,
