...
const obtenerRolesPermisos = async (req, res = response) => {
  try {
    ...
    // Si el modelo soporta paginado, usarlo; aquí findAll no acepta params, así que paginamos en memoria
    const all = await AdmRolPermiso.findAll();
    const total = Array.isArray(all) ? all.length : 0;
    const data = Array.isArray(all) ? all.slice(desde, desde + limite) : all;
    res.json({ ok: true, roles_permisos: data, total, page, per_page, desde, limite });
  } catch (error) {
