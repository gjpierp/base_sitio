/**
 * Helper: registrar-accion.js
 * Propósito: Proveer middleware/factory para registrar acciones del sistema
 * en la tabla de auditoría. Usado en rutas para anotar CREAR/ACTUALIZAR/ELIMINAR.
 */
let HistorialAcciones = require("../models/adm_historial_acciones");
// Manejar posible export por defecto (interop) o módulos incompletos por circular requires
if (HistorialAcciones && typeof HistorialAcciones.default !== "undefined") {
  HistorialAcciones = HistorialAcciones.default;
}

const registrarAccion = async (
  idUsuario,
  accion,
  entidad,
  idEntidad = null,
  descripcion = null,
  req = null
) => {
  try {
    const ipOrigen = req
      ? req.headers["x-forwarded-for"] || req.connection.remoteAddress
      : null;

    // Compatibilidad: preferir .registrar, fallback a .create
    const registrarFn =
      HistorialAcciones &&
      (HistorialAcciones.registrar || HistorialAcciones.create);
    if (typeof registrarFn === "function") {
      // La tabla `adm_historial_acciones` utiliza columnas:
      // id_accion, id_usuario, accion, descripcion, fecha, direccion_ip
      // Ensure we always provide a value for id_usuario (DB column NOT NULL)
      // Prefer the authenticated user id, fallback to affected entity id, else 0 (system)
      const resolvedIdUsuario = idUsuario || idEntidad || 0;
      if (!idUsuario) {
        console.debug(
          "[registrarAccion] idUsuario ausente, usando fallback",
          resolvedIdUsuario
        );
      }
      const payload = {
        id_usuario: resolvedIdUsuario,
        accion,
        descripcion: descripcion || `${accion} en ${entidad}`,
        direccion_ip: ipOrigen,
      };
      try {
        await registrarFn.call(HistorialAcciones, payload);
        return;
      } catch (err) {
        // Loguear error sin bloquear la respuesta principal
        console.error("Error al escribir historial de acciones:", err);
      }
    } else {
      console.error(
        "HistorialAcciones.registrar no disponible. Valor:",
        HistorialAcciones
      );
    }
  } catch (error) {
    // No lanzar error para no afectar la operación principal
    console.error("Error al registrar acción:", error);
  }
};

// Middleware para registrar acciones automáticamente
const middlewareRegistrarAccion = (accion, entidad) => {
  return async (req, res, next) => {
    // Guardar el método send original
    const originalSend = res.send;

    // Sobrescribir el método send
    res.send = function (data) {
      // Intentar parsear la respuesta si es JSON
      let responseData;
      try {
        responseData = typeof data === "string" ? JSON.parse(data) : data;
      } catch (e) {
        responseData = data;
      }

      // Si la respuesta es exitosa, registrar la acción
        if (responseData && responseData.ok) {
          const idUsuario = req.uid;

          // Extraer id de entidad creada/afectada intentando varias rutas posibles
          const tryPaths = [
            () => req.params && req.params.id,
            () => responseData && responseData.id_usuario,
            () => responseData && responseData.id,
            () => responseData && responseData.insertId,
            () => responseData && responseData.usuario && responseData.usuario.id_usuario,
            () => responseData && responseData.usuario && responseData.usuario.id,
            () => responseData && responseData.data && responseData.data.id,
            () => responseData && responseData.id_rol,
            () => responseData && responseData.id_permiso,
            () => responseData && responseData.id_menu,
          ];

          let idEntidad = null;
          for (const fn of tryPaths) {
            try {
              const v = fn();
              if (v !== undefined && v !== null && String(v) !== "") {
                idEntidad = v;
                break;
              }
            } catch (e) {
              // ignore
            }
          }

          const descripcion = `${accion} en ${entidad}`;

          if (!idUsuario && !idEntidad) {
            console.debug(
              '[registrarAccion] no se encontró idUsuario ni idEntidad; evitar insertar 0 para FK'
            );
          }

          registrarAccion(
            idUsuario,
            accion,
            entidad,
            idEntidad,
            descripcion,
            req
          );
        }

      // Llamar al método send original
      return originalSend.call(this, data);
    };

    next();
  };
};

module.exports = {
  registrarAccion,
  middlewareRegistrarAccion,
};
