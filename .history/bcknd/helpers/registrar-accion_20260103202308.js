...
const registrarAccion = async (
  ...
  try {
    ...
    const registrarFn =
      HistorialAcciones && (HistorialAcciones.registrar || HistorialAcciones.create);
    if (typeof registrarFn === "function") {
      // Intento primario: esquema tradicional (entidad, id_entidad, ip_origen)
      try {
        ...
        // Si la tabla tiene columnas distintas (p. ej. `tabla`, `id_registro`, `direccion_ip`), reintentar con mapeo alternativo
        if (err && err.code === "ER_BAD_FIELD_ERROR") {
          try {
            await registrarFn.call(HistorialAcciones, {
              ...
              accion,
              tabla: entidad,
              id_registro: idEntidad,
              datos_nuevos: descripcion,
              direccion_ip: ipOrigen,
            });
            return;
          } catch (err2) {
            // seguir para el logging fuera
            console.error("Registrar acción - reintento alternativo falló:", err2);
          }
        }
        // Si no era por columnas faltantes, re-lanzar para loguearlo más abajo
        throw err;
      }
    } else {
      console.error("HistorialAcciones.registrar no disponible. Valor:", HistorialAcciones);
    }
  } catch (error) {
