...
const validarPermiso = (...permisosRequeridos) => {
  return async (req, res = response, next) => {
    ...
    try {
      ...
      if (esAdmin) {
        console.debug('[validarPermiso] uid=', uid, 'esAdmin=true — permitiendo bypass');
        return next();
      }
      ...
      try {
        ...
        console.debug('[validarPermiso] rolesUsuario=', rolesUsuario.map((r) => r.nombre));
      } catch (e) {
        // ignore
      }
      ...
      if (!tienePermiso) {
        console.debug('[validarPermiso] uid=', uid, 'permisos=', Array.from(permisoIdents), 'requeridos=', permisosRequeridos);
        return res.status(403).json({ ok: false, msg: "No tiene permisos suficientes para realizar esta acción" });
      }
...
const validarRol = (...rolesRequeridos) => {
  return async (req, res = response, next) => {
    ...
    try {
      ...
      const tieneRol = rolesRequeridos.some((rol) => nombresRoles.includes(rol));

      if (!tieneRol) {
        console.debug('[validarRol] uid=', uid, 'roles=', nombresRoles, 'requeridos=', rolesRequeridos);
        return res.status(403).json({ ok: false, msg: "No tiene el rol necesario para realizar esta acción" });
      }
...
const validarPermisoOAdmin = (...permisosRequeridos) => {
  return async (req, res = response, next) => {
    ...
    try {
      ...
      const tienePermiso = permisosRequeridos.some((permiso) => permisoIdents.has(String(permiso).toUpperCase()));

      if (!tienePermiso) {
        console.debug('[validarPermisoOAdmin] uid=', uid, 'esAdmin=', esAdmin, 'permisos=', Array.from(permisoIdents), 'requeridos=', permisosRequeridos);
        return res.status(403).json({ ok: false, msg: "No tiene permisos suficientes para realizar esta acción" });
      }
