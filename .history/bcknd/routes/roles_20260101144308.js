...
// GET /api/roles/buscar/:q - Buscar roles por texto (nombre/descripcion)
router.get("/buscar/:q", [validarPermiso("ROLES_VER")], (req, res) => {
  req.params.tabla = "roles";
  req.params.busqueda = req.params.q;
  return getDocumentosColeccion(req, res);
});

// GET /api/roles/buscar/id/:id - Buscar rol por id (alias a /:id)
router.get(
  "/buscar/id/:id",
  [campoNumerico("id", "El id debe ser numérico"), validarCampos, validarPermiso("ROLES_VER")],
  obtenerRol
);

// GET /api/roles/buscar/nombre/:nombre - Buscar rol por nombre exacto/filtrado
router.get(
