...
router.get("/buscar/:q", [validarPermiso("PERMISOS_VER")], (req, res) => {
  req.params.tabla = "permisos";
  req.params.busqueda = req.params.q;
  return getDocumentosColeccion(req, res);
});

// GET /api/permisos/buscar/id/:id - Buscar permiso por id (alias a /:id)
router.get(
  "/buscar/id/:id",
  [campoNumerico("id", "El id debe ser numérico"), validarCampos, validarPermiso("PERMISOS_VER")],
  obtenerPermiso
);

// GET /api/permisos/buscar/nombre/:nombre - Buscar permiso por nombre/descripcion
router.get(
