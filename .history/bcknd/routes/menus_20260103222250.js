...
router.get("/usuario", obtenerArbolMenus);
...
router.get("/buscar/:q", [validarPermiso("MENUS_VER")], (req, res) => {
  req.params.tabla = "menus";
  req.params.busqueda = req.params.q;
  return getDocumentosColeccion(req, res);
});

// GET /api/menus/buscar/id/:id - Buscar menú por id (alias a /:id)
router.get(
  "/buscar/id/:id",
  [[...validarIdMenu], validarCampos, validarPermiso("MENUS_VER")],
  obtenerMenu
);

// GET /api/menus/buscar/nombre/:nombre - Buscar menús por nombre (texto)
router.get(
...
// DELETE /api/menus/:id/permisos/:permId - Eliminar permiso de un menú
router.delete(
  "/permisos/:id/:permId",
  [...validarIdMenu, validarPermiso("MENUS_EDITAR")],
  eliminarPermisoMenu
);
