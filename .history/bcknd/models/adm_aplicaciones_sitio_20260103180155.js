...
class AdmAplicacionesSitio extends BaseModel {
  static async findByClave(clave) {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_aplicaciones_sitio WHERE clave = ? AND activo = 1",
      [clave]
    );
    return rows[0] || null;
  }
  ...
  static async findById(id_aplicacion) {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_aplicaciones_sitio WHERE id_aplicacion = ?",
      [id_aplicacion]
    );
    return rows[0] || null;
  }
  ...
  static async findAllBySitio(id_sitio = null) {
    ...
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_aplicaciones_sitio WHERE id_sitio = ? AND activo = 1 ORDER BY 1 DESC",
      [id_sitio]
    );
    return rows;
  }
  ...
  static async findAll() {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_aplicaciones_sitio WHERE activo = 1 ORDER BY 1 DESC"
    );
    return rows;
  }
  ...
  static async listar(
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query(
        "SELECT * FROM adm_aplicaciones_sitio " + whereSql + " " + orderSql,
        vals
      );
      return rows;
    }
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_aplicaciones_sitio " +
        whereSql +
        " " +
        orderSql +
        " LIMIT ? OFFSET ?",
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query(
      "SELECT COUNT(*) as total FROM adm_aplicaciones_sitio " + whereSql,
      vals
    );
  ...
  static async create(data) {
    ...
    const sql =
      "INSERT INTO adm_aplicaciones_sitio (" +
      keys.join(",") +
      ") VALUES (" +
      keys.map((_) => "?").join(",") +
      ")";
  ...
  static async update(id_aplicacion, data) {
    ...
    await BaseModel.query(
      "UPDATE adm_aplicaciones_sitio SET " +
        setSql +
        " WHERE id_aplicacion = ?",
      vals.concat([id_aplicacion])
    );
  }

  static async delete(id_aplicacion) {
    await BaseModel.query(
      "DELETE FROM adm_aplicaciones_sitio WHERE id_aplicacion = ?",
      [id_aplicacion]
    );
  }
  ...
  static async softDelete(id_aplicacion) {
    await BaseModel.query(
      "UPDATE adm_aplicaciones_sitio SET activo = 0 WHERE id_aplicacion = ?",
      [id_aplicacion]
    );
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_aplicaciones_sitio " + whereSql,
      vals
    );
    return rows;
  }
