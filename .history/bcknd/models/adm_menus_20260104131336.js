...
class AdmMenus extends BaseModel {
  ...
  static async findAll() {
    const [rows] = await BaseModel.query('SELECT * FROM adm_menus WHERE visible = 1 ORDER BY orden ASC');
    return rows;
  }
  ...
  static async softDelete(id_menu) {
    await BaseModel.query('UPDATE adm_menus SET visible = 0 WHERE id_menu = ?', [id_menu]); 
  }
...
AdmMenus.buildUserMenuTree = async function (id_usuario = null) {
  ...
  const filtered = menus.filter((m) => {
    try {
      ...
      if (typeof m.activo !== "undefined" && Number(m.activo) !== 1)
        return false;
      // Si existe la columna `visible`, incluir sólo visible==1
      if (typeof m.visible !== "undefined" && Number(m.visible) !== 1)
        return false;
    } catch (e) {
  ...
  for (const idStr of Object.keys(map)) {
    ...
    const parentId =
      node.id_menu_padre ?? node.id_padre ?? node.parent_id ?? null;
  ...
  const sortChildren = (arr) => {
    ...
    for (const it of arr)
      if (Array.isArray(it.hijos) && it.hijos.length) sortChildren(it.hijos);
  };
  sortChildren(roots);
