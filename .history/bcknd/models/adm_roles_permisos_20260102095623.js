...
class AdmRolPermiso {
  ...
  static async listar(
    ...
    if (desde === undefined || limite === undefined) {
      const [rows] = await db.query(
        `SELECT * FROM adm_roles_permisos ORDER BY ${col} ${dir}`
      );
      return rows;
    }
    const [rows] = await db.query(
      `SELECT * FROM adm_roles_permisos ORDER BY ${col} ${dir} LIMIT ? OFFSET ?`,
      [limite, desde]
    );
    return rows;
  }
  ...
  static async contar() {
    const [rows] = await db.query(
      "SELECT COUNT(*) as total FROM adm_roles_permisos"
    );
