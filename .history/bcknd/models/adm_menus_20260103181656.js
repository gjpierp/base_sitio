...
class AdmMenus extends BaseModel {
  static async findById(id_menu) {
    const [rows] = await BaseModel.query('SELECT * FROM adm_menus WHERE id_menu = ?', [id_menu]);
    return rows[0] || null;
  }

  static async findAll() {
    const [rows] = await BaseModel.query('SELECT * FROM adm_menus  ORDER BY 1 DESC');
    return rows;
  }
  ...
  static async listar(desde = 0, limite = 10, sortColumn = null, sortDir = "ASC", filters = {}) {
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query('SELECT * FROM adm_menus ' + whereSql + ' ' + orderSql , vals);
      return rows;
    }
    const [rows] = await BaseModel.query('SELECT * FROM adm_menus ' + whereSql + ' ' + orderSql + ' LIMIT ? OFFSET ?', vals.concat([limite, desde]));
    return rows;
  }
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query('SELECT COUNT(*) as total FROM adm_menus ' + whereSql, vals);
  ...
  static async create(data) {
    ...
    const sql = 'INSERT INTO adm_menus (' + keys.join(',') + ') VALUES (' + keys.map(_=>'?').join(',') + ')';
  ...
  static async update(id_menu, data) {
    ...
    const vals = keys.map(k=>data[k]);
    await BaseModel.query('UPDATE adm_menus SET ' + setSql + ' WHERE id_menu = ?', vals.concat([id_menu]));
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query('SELECT * FROM adm_menus ' + whereSql, vals);
    return rows;
  }
}
...
AdmMenus.existsByNombre = async function (nombre, excludeId) {
  const [rows] = await require('./BaseModel').query(
    'SELECT 1 FROM adm_menus WHERE nombre = ?' + (excludeId ? ' AND id_menu != ?' : ' LIMIT 1'),
...
AdmMenus.existsByUrl = async function (url, excludeId) {
  const [rows] = await require('./BaseModel').query(
    'SELECT 1 FROM adm_menus WHERE url = ?' + (excludeId ? ' AND id_menu != ?' : ' LIMIT 1'),
...
AdmMenus.findByIds = async function (ids) {
  ...
  const [rows] = await require('./BaseModel').query(`SELECT * FROM adm_menus WHERE id_menu IN (${placeholders})`, ids);
  return rows;
};

// Obtener jerarquía simple: hijos directos
AdmMenus.obtenerJerarquia = async function (id_menu) {
  const [rows] = await require('./BaseModel').query('SELECT * FROM adm_menus WHERE id_menu_padre = ?', [id_menu]);
  return rows || [];
};
