...
class AdmTiposUsuario extends BaseModel {
  static get tableName() { return 'adm_tipos_usuario'; }
  static get primaryKey() { return 'id_tipo_usuario'; }
  ...
  static async findAll() {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_tipos_usuario WHERE activo = 1 ORDER BY 1 DESC"
    );
    return rows;
  }
  ...
  static async listar(
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query(
        "SELECT * FROM adm_tipos_usuario " + whereSql + " " + orderSql,
        vals
      );
      return rows;
    }
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_tipos_usuario " +
        whereSql +
        " " +
        orderSql +
        " LIMIT ? OFFSET ?",
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query(
      "SELECT COUNT(*) as total FROM adm_tipos_usuario " + whereSql,
      vals
    );
  ...
  static async update(id_tipo_usuario, data) {
    ...
    await BaseModel.query(
      "UPDATE adm_tipos_usuario SET " + setSql + " WHERE id_tipo_usuario = ?",
      vals.concat([id_tipo_usuario])
    );
  }

  static async delete(id_tipo_usuario) {
    await BaseModel.query(
      "DELETE FROM adm_tipos_usuario WHERE id_tipo_usuario = ?",
      [id_tipo_usuario]
    );
  }
  ...
  static async softDelete(id_tipo_usuario) {
    await BaseModel.query(
      "UPDATE adm_tipos_usuario SET activo = 0 WHERE id_tipo_usuario = ?",
      [id_tipo_usuario]
    );
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_tipos_usuario " + whereSql,
      vals
    );
    return rows;
  }
