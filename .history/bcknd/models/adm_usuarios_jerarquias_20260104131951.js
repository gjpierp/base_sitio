...
class AdmUsuariosJerarquias extends BaseModel {
  static get tableName() { return 'adm_usuarios_jerarquias'; }
  static get primaryKey() { return 'id_usuario_jerarquia'; }
  ...
  static async findAll() {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_usuarios_jerarquias WHERE activo = 1 ORDER BY 1 DESC"
    );
    return rows;
  }
  ...
  static async listar(
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query(
        "SELECT * FROM adm_usuarios_jerarquias " + whereSql + " " + orderSql,
        vals
      );
      return rows;
    }
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_usuarios_jerarquias " +
        whereSql +
        " " +
        orderSql +
        " LIMIT ? OFFSET ?",
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query(
      "SELECT COUNT(*) as total FROM adm_usuarios_jerarquias " + whereSql,
      vals
    );
  ...
  static async create(data) {
    ...
    const sql =
      "INSERT INTO adm_usuarios_jerarquias (" +
      keys.join(",") +
      ") VALUES (" +
      keys.map((_) => "?").join(",") +
      ")";
  ...
  static async update(id_usuario_jerarquia, data) {
    ...
    await BaseModel.query(
      "UPDATE adm_usuarios_jerarquias SET " +
        setSql +
        " WHERE id_usuario_jerarquia = ?",
      vals.concat([id_usuario_jerarquia])
    );
  }
  ...
  static async softDelete(id_usuario_jerarquia) {
    await BaseModel.query(
      "UPDATE adm_usuarios_jerarquias SET activo = 0 WHERE id_usuario_jerarquia = ?",
      [id_usuario_jerarquia]
    );
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_usuarios_jerarquias " + whereSql,
      vals
    );
    return rows;
  }
