...
class AdmHistorialContrasena {
  ...
  static async listar(
    desde = 0,
    limite = 10,
    sortColumn = "id_historial",
    sortDir = "DESC"
  ) {
    const col = ["id_historial", "id_usuario", "fecha_cambio"].includes(
      sortColumn
    )
      ? sortColumn
      : "id_historial";
    ...
    if (desde === undefined || limite === undefined) {
      const [rows] = await db.query(
        `SELECT * FROM adm_historial_contrasenas ORDER BY ${col} ${dir}`
      );
      return rows;
    }
    const [rows] = await db.query(
      `SELECT * FROM adm_historial_contrasenas ORDER BY ${col} ${dir} LIMIT ? OFFSET ?`,
      [limite, desde]
    );
    return rows;
  }
  ...
  static async contar() {
    const [rows] = await db.query(
      "SELECT COUNT(*) as total FROM adm_historial_contrasenas"
    );
    return rows[0]?.total || 0;
  }
