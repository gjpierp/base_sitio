...
class AdmUsuarios extends BaseModel {
  static get tableName() { return 'adm_usuarios'; }
  ...
  static async sincronizarRoles(id_usuario, roles = []) {
    ...
    for (const rel of toDeactivate) {
      try { await AdmUsuarioRol.update(rel.id_usuario_rol, { activo: 0 }); } catch (e) { console.error('[sincronizarRoles] error deactivating role relation', e); }
    }

    for (const roleId of toAdd) {
      try { await AdmUsuarioRol.create({ id_usuario, id_rol: Number(roleId), activo: 1 }); } catch (e) { console.error('[sincronizarRoles] error creating role relation', e); }
    }
  }
  ...
  static async findById(id_usuario) {
    const [rows] = await BaseModel.query('SELECT * FROM adm_usuarios WHERE id_usuario = ?', [id_usuario]);
    return rows[0] || null;
  }

  static async findAll() {
    const [rows] = await BaseModel.query('SELECT * FROM adm_usuarios WHERE activo = 1 ORDER BY 1 DESC');
    return rows;
  }
  ...
  static async listar(desde = 0, limite = 10, sortColumn = null, sortDir = 'ASC', filters = {}) {
    ...
    if (typeof desde === 'undefined' || typeof limite === 'undefined') {
      const [rows] = await BaseModel.query('SELECT * FROM adm_usuarios ' + whereSql + ' ' + orderSql, vals);
      return rows;
    }
    const [rows] = await BaseModel.query('SELECT * FROM adm_usuarios ' + whereSql + ' ' + orderSql + ' LIMIT ? OFFSET ?', vals.concat([limite, desde]));
    return rows;
  }
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query('SELECT COUNT(*) as total FROM adm_usuarios ' + whereSql, vals);
  ...
  static async create(data) {
    ...
    const sql = 'INSERT INTO adm_usuarios (' + keys.join(',') + ') VALUES (' + keys.map(_ => '?').join(',') + ')';
  ...
  static async update(id_usuario, data) {
    ...
    const vals = keys.map((k) => data[k]);
    await BaseModel.query('UPDATE adm_usuarios SET ' + setSql + ' WHERE id_usuario = ?', vals.concat([id_usuario]));
  }

  static async delete(id_usuario) {
    await BaseModel.query('DELETE FROM adm_usuarios WHERE id_usuario = ?', [id_usuario]);
  }

  static async softDelete(id_usuario) {
    await BaseModel.query('UPDATE adm_usuarios SET activo = 0 WHERE id_usuario = ?', [id_usuario]);
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? 'WHERE ' + where.join(' AND ') : '';
    const [rows] = await BaseModel.query('SELECT * FROM adm_usuarios ' + whereSql, vals);
    return rows;
  }
}
...
AdmUsuarios.obtenerPorCorreo = async function (correo) {
  ...
  try {
    const [rows] = await BaseModel.query('SELECT * FROM adm_usuarios WHERE correo = ? LIMIT 1', [correo]);
    if (Array.isArray(rows) && rows.length) return rows[0];
  } catch (e) {}
  try {
    const [rows2] = await BaseModel.query('SELECT * FROM adm_usuarios WHERE nombre_usuario = ? LIMIT 1', [correo]);
    if (Array.isArray(rows2) && rows2.length) return rows2[0];
  } catch (e) {}
  return null;
};
AdmUsuarios.obtenerPorId = function (id) { return AdmUsuarios.findById(id); };
AdmUsuarios.obtenerRolesPorUsuario = function (id_usuario) { return AdmUsuarios.obtenerRoles(id_usuario); };
AdmUsuarios.obtenerPermisosPorUsuario = function (id_usuario) { return AdmUsuarios.obtenerPermisos(id_usuario); };
