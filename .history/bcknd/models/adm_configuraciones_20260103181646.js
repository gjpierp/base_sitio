...
class AdmConfiguraciones extends BaseModel {
  static async findByClave(clave) {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_configuraciones WHERE clave = ?",
      [clave]
    );
    return rows[0] || null;
  }
  ...
  static async findById(id_configuracion) {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_configuraciones WHERE id_configuracion = ?",
      [id_configuracion]
    );
    return rows[0] || null;
  }
  ...
  static async findAll() {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_configuraciones  ORDER BY 1 DESC"
    );
    return rows;
  }
  ...
  static async listar(
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query(
        "SELECT * FROM adm_configuraciones " + whereSql + " " + orderSql,
        vals
      );
      return rows;
    }
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_configuraciones " +
        whereSql +
        " " +
        orderSql +
        " LIMIT ? OFFSET ?",
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query(
      "SELECT COUNT(*) as total FROM adm_configuraciones " + whereSql,
      vals
    );
  ...
  static async update(id_configuracion, data) {
    ...
    await BaseModel.query(
      "UPDATE adm_configuraciones SET " +
        setSql +
        " WHERE id_configuracion = ?",
      vals.concat([id_configuracion])
    );
  }

  static async delete(id_configuracion) {
    await BaseModel.query(
      "DELETE FROM adm_configuraciones WHERE id_configuracion = ?",
      [id_configuracion]
    );
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_configuraciones " + whereSql,
      vals
    );
    return rows;
  }
