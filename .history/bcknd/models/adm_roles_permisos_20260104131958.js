...
class AdmRolesPermisos extends BaseModel {
  static get tableName() { return 'adm_roles_permisos'; }
  static get primaryKey() { return 'id_rol_permiso'; }
  ...
  static async findAll() {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_roles_permisos WHERE activo = 1 ORDER BY 1 DESC"
    );
    return rows;
  }
  ...
  static async listar(
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query(
        "SELECT * FROM adm_roles_permisos " + whereSql + " " + orderSql,
        vals
      );
      return rows;
    }
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_roles_permisos " +
        whereSql +
        " " +
        orderSql +
        " LIMIT ? OFFSET ?",
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query(
      "SELECT COUNT(*) as total FROM adm_roles_permisos " + whereSql,
      vals
    );
  ...
  static async create(data) {
    ...
    const sql =
      "INSERT INTO adm_roles_permisos (" +
      keys.join(",") +
      ") VALUES (" +
      keys.map((_) => "?").join(",") +
      ")";
  ...
  static async update(id_rol_permiso, data) {
    ...
    await BaseModel.query(
      "UPDATE adm_roles_permisos SET " + setSql + " WHERE id_rol_permiso = ?",
      vals.concat([id_rol_permiso])
    );
  }

  static async delete(id_rol_permiso) {
    await BaseModel.query(
      "DELETE FROM adm_roles_permisos WHERE id_rol_permiso = ?",
      [id_rol_permiso]
    );
  }
  ...
  static async softDelete(id_rol_permiso) {
    await BaseModel.query(
      "UPDATE adm_roles_permisos SET activo = 0 WHERE id_rol_permiso = ?",
      [id_rol_permiso]
    );
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_roles_permisos " + whereSql,
      vals
    );
    return rows;
  }
