...
class AdmTemas {
  ...
  static async listar(desde = 0, limite = 10, sortColumn = "nombre", sortDir = "ASC", publicoOnly = true) {
    ...
    if (desde === undefined || limite === undefined) {
      const [rows] = await db.query(`SELECT * FROM adm_temas ${baseWhere} ORDER BY ${col} ${dir}`);
      return rows.map((r) => ({ ...r, css_vars: AdmTemas._parseCssVars(r.css_vars) }));
    }
    const [rows] = await db.query(`SELECT * FROM adm_temas ${baseWhere} ORDER BY ${col} ${dir} LIMIT ? OFFSET ?`, [limite, desde]);
    return rows.map((r) => ({ ...r, css_vars: AdmTemas._parseCssVars(r.css_vars) }));
  }

  static async contar(publicoOnly = true) {
    const baseWhere = publicoOnly ? "WHERE activo = 1 AND publico = 1" : "";
    const [rows] = await db.query(`SELECT COUNT(*) as total FROM adm_temas ${baseWhere}`);
