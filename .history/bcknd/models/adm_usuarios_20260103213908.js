...
class AdmUsuarios extends BaseModel {
  ...
  static async tieneRol(id_usuario, rolNombre) {
    ...
    const [rows] = await BaseModel.query(
      `SELECT 1 FROM adm_roles r JOIN adm_usuarios_roles ur ON ur.id_rol = r.id_rol WHERE ur.id_usuario = ? AND (LOWER(r.nombre) = LOWER(?) OR LOWER(r.codigo) = LOWER(?) OR LOWER(r.nombre) = LOWER(?)) LIMIT 1`,
      [id_usuario, variants[0], variants[0], variants[1]]
    );
