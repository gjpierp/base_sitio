...
class AdmUsuarioRol {
  ...
  static async listar(desde = 0, limite = 10, sortColumn = "id_usuario_rol", sortDir = "ASC") {
    ...
    if (desde === undefined || limite === undefined) {
      const [rows] = await db.query(`SELECT * FROM adm_usuarios_roles ORDER BY ${col} ${dir}`);
      return rows;
    }
    const [rows] = await db.query(`SELECT * FROM adm_usuarios_roles ORDER BY ${col} ${dir} LIMIT ? OFFSET ?`, [limite, desde]);
    return rows;
  }

  static async contar() {
    const [rows] = await db.query("SELECT COUNT(*) as total FROM adm_usuarios_roles");
