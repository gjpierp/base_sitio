...
class AdmUsuariosAplicaciones {
  static async findById(id_usuario_aplicacion) {
    const [rows] = await db.query(
      "SELECT * FROM adm_usuarios_aplicaciones WHERE id_usuario_aplicacion = ?",
  ...
  static async findAll() {
    const [rows] = await db.query(
      "SELECT * FROM adm_usuarios_aplicaciones ORDER BY id_usuario_aplicacion DESC"
    );
    return rows;
  }
  ...
  static async listar(desde = 0, limite = 10) {
    ...
    const [rows] = await db.query(
      "SELECT * FROM adm_usuarios_aplicaciones ORDER BY id_usuario_aplicacion DESC LIMIT ? OFFSET ?",
      [limite, desde]
    );
    return rows;
  }
  ...
  static async contar() {
    const [rows] = await db.query(
      "SELECT COUNT(*) as total FROM adm_usuarios_aplicaciones"
    );
  ...
  static async delete(id_usuario_aplicacion) {
    await db.query(
      "DELETE FROM adm_usuarios_aplicaciones WHERE id_usuario_aplicacion = ?",
      [id_usuario_aplicacion]
    );
  }
  ...
  static async softDelete(id_usuario_aplicacion) {
    await db.query(
      "UPDATE adm_usuarios_aplicaciones SET activo = 0 WHERE id_usuario_aplicacion = ?",
      [id_usuario_aplicacion]
    );
  }
}
