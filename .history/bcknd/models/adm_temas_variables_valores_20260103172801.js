...
class AdmTemasVariablesValores {
  static async findById(id) {
    const [rows] = await db.query(
      "SELECT * FROM adm_temas_variables_valores WHERE id = ?",
      [id]
    );
    return rows[0] || null;
  }
  ...
  static async findAll() {
    const [rows] = await db.query(
      "SELECT * FROM adm_temas_variables_valores ORDER BY id DESC"
    );
    return rows;
  }
  ...
  static async listar(desde = 0, limite = 10) {
    ...
    const [rows] = await db.query(
      "SELECT * FROM adm_temas_variables_valores ORDER BY id DESC LIMIT ? OFFSET ?",
      [limite, desde]
    );
    return rows;
  }
  ...
  static async delete(id) {
    await db.query("DELETE FROM adm_temas_variables_valores WHERE id = ?", [
      id,
    ]);
  }
}
