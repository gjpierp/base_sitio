...
class AdmHistorialAcciones extends BaseModel {
  static async findById(id_accion) {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_acciones WHERE id_accion = ?",
      [id_accion]
    );
    return rows[0] || null;
  }
  ...
  static async findAll() {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_acciones  ORDER BY 1 DESC"
    );
    return rows;
  }
  ...
  static async listar(
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query(
        "SELECT * FROM adm_historial_acciones " + whereSql + " " + orderSql,
        vals
      );
      return rows;
    }
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_acciones " +
        whereSql +
        " " +
        orderSql +
        " LIMIT ? OFFSET ?",
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query(
      "SELECT COUNT(*) as total FROM adm_historial_acciones " + whereSql,
      vals
    );
  ...
  static async update(id_accion, data) {
    ...
    await BaseModel.query(
      "UPDATE adm_historial_acciones SET " + setSql + " WHERE id_accion = ?",
      vals.concat([id_accion])
    );
  }

  static async delete(id_accion) {
    await BaseModel.query(
      "DELETE FROM adm_historial_acciones WHERE id_accion = ?",
      [id_accion]
    );
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_acciones " + whereSql,
      vals
    );
    return rows;
  }
...
AdmHistorialAcciones.search = function (filters, desde = 0, limite = 10) {
  return AdmHistorialAcciones.listar(
    Number(desde) || 0,
    Number(limite) || 10,
    null,
    "ASC",
...
// Compatibilidad: registrar acción (alias de create)
AdmHistorialAcciones.registrar = function (data) {
  return AdmHistorialAcciones.create(data);
};
