...
class AdmRoles extends BaseModel {
  static async findById(id_rol) {
    const [rows] = await BaseModel.query('SELECT * FROM adm_roles WHERE id_rol = ?', [id_rol]);
    return rows[0] || null;
  }

  static async findAll() {
    const [rows] = await BaseModel.query('SELECT * FROM adm_roles WHERE activo = 1 ORDER BY 1 DESC');
    return rows;
  }
  ...
  static async listar(desde = 0, limite = 10, sortColumn = null, sortDir = "ASC", filters = {}) {
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query('SELECT * FROM adm_roles ' + whereSql + ' ' + orderSql , vals);
      return rows;
    }
    const [rows] = await BaseModel.query('SELECT * FROM adm_roles ' + whereSql + ' ' + orderSql + ' LIMIT ? OFFSET ?', vals.concat([limite, desde]));
    return rows;
  }
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query('SELECT COUNT(*) as total FROM adm_roles ' + whereSql, vals);
  ...
  static async create(data) {
    ...
    const sql = 'INSERT INTO adm_roles (' + keys.join(',') + ') VALUES (' + keys.map(_=>'?').join(',') + ')';
  ...
  static async update(id_rol, data) {
    ...
    const vals = keys.map(k=>data[k]);
    await BaseModel.query('UPDATE adm_roles SET ' + setSql + ' WHERE id_rol = ?', vals.concat([id_rol]));
  }
  ...
  static async softDelete(id_rol) {
    await BaseModel.query('UPDATE adm_roles SET activo = 0 WHERE id_rol = ?', [id_rol]); 
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query('SELECT * FROM adm_roles ' + whereSql, vals);
    return rows;
  }
}
...
AdmRoles.sincronizarPermisos = async function (id_rol, permisos = []) {
  ...
  const toDeactivate = existingActive.filter((r) => !desired.includes(Number(r.id_permiso)));
  ...
  for (const permId of toAdd) {
    try {
      await AdmRolesPermisos.create({ id_rol, id_permiso: Number(permId), activo: 1 });
    } catch (e) {
