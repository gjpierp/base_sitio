...
class AdmHistorialAccesos extends BaseModel {
  static async findById(id_acceso) {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_accesos WHERE id_acceso = ?",
      [id_acceso]
    );
    return rows[0] || null;
  }
  ...
  static async findAll() {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_accesos  ORDER BY 1 DESC"
    );
    return rows;
  }
  ...
  static async listar(
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query(
        "SELECT * FROM adm_historial_accesos " + whereSql + " " + orderSql,
        vals
      );
      return rows;
    }
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_accesos " +
        whereSql +
        " " +
        orderSql +
        " LIMIT ? OFFSET ?",
  ...
  static async contar(filters = {}) {
    ...
    const [rows] = await BaseModel.query(
      "SELECT COUNT(*) as total FROM adm_historial_accesos " + whereSql,
      vals
    );
  ...
  static async create(data) {
    ...
    const sql =
      "INSERT INTO adm_historial_accesos (" +
      keys.join(",") +
      ") VALUES (" +
      keys.map((_) => "?").join(",") +
      ")";
  ...
  static async update(id_acceso, data) {
    ...
    await BaseModel.query(
      "UPDATE adm_historial_accesos SET " + setSql + " WHERE id_acceso = ?",
      vals.concat([id_acceso])
    );
  }

  static async delete(id_acceso) {
    await BaseModel.query(
      "DELETE FROM adm_historial_accesos WHERE id_acceso = ?",
      [id_acceso]
    );
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_accesos " + whereSql,
      vals
    );
    return rows;
  }
...
AdmHistorialAccesos.search = function (filters, desde = 0, limite = 10) {
  return AdmHistorialAccesos.listar(
    Number(desde) || 0,
    Number(limite) || 10,
    null,
    "ASC",
