...
class AdmHistorialContrasenas extends BaseModel {
  ...
  static async findAll() {
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_contrasenas  ORDER BY 1 DESC"
    );
    return rows;
  }
  ...
  static async listar(
    ...
    if (typeof desde === "undefined" || typeof limite === "undefined") {
      const [rows] = await BaseModel.query(
        "SELECT * FROM adm_historial_contrasenas " + whereSql + " " + orderSql,
        vals
      );
      return rows;
    }
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_contrasenas " +
        whereSql +
        " " +
        orderSql +
        " LIMIT ? OFFSET ?",
  ...
  static async create(data) {
    ...
    const sql =
      "INSERT INTO adm_historial_contrasenas (" +
      keys.join(",") +
      ") VALUES (" +
      keys.map((_) => "?").join(",") +
      ")";
  ...
  static async update(id_historial, data) {
    ...
    await BaseModel.query(
      "UPDATE adm_historial_contrasenas SET " +
        setSql +
        " WHERE id_historial = ?",
      vals.concat([id_historial])
    );
  }
  ...
  static async findBy(filters = {}) {
    ...
    const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";
    const [rows] = await BaseModel.query(
      "SELECT * FROM adm_historial_contrasenas " + whereSql,
      vals
    );
    return rows;
  }
...
AdmHistorialContrasenas.search = function (filters, desde = 0, limite = 10) {
  return AdmHistorialContrasenas.listar(
    Number(desde) || 0,
    Number(limite) || 10,
    null,
    "ASC",
