...
class AdmMenuPermiso {
  ...
  static async deleteByMenuId(id_menu) {
    await db.query("DELETE FROM adm_menus_permisos WHERE id_menu = ?", [
      id_menu,
    ]);
  }
  ...
  static async findByMenuId(id_menu) {
    const [rows] = await db.query(
      "SELECT * FROM adm_menus_permisos WHERE id_menu = ?",
      [id_menu]
    );
    return rows;
  }

  static async deleteByMenuAndPerm(id_menu, id_permiso) {
    await db.query("DELETE FROM adm_menus_permisos WHERE id_menu = ? AND id_permiso = ?", [
      id_menu,
      id_permiso,
    ]);
  }
