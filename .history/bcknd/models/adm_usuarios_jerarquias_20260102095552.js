...
class AdmUsuarioJerarquia {
  ...
  static async listar(desde = 0, limite = 10, sortColumn = "id_usuario_jerarquia", sortDir = "ASC") {
    const allowed = ["id_usuario_jerarquia", "id_usuario", "id_jerarquia", "activo"];
    const col = allowed.includes(sortColumn) ? sortColumn : "id_usuario_jerarquia";
    const dir = String(sortDir).toUpperCase() === "DESC" ? "DESC" : "ASC";
    if (desde === undefined || limite === undefined) {
      const [rows] = await db.query(`SELECT * FROM adm_usuarios_jerarquias ORDER BY ${col} ${dir}`);
      return rows;
    }
    const [rows] = await db.query(`SELECT * FROM adm_usuarios_jerarquias ORDER BY ${col} ${dir} LIMIT ? OFFSET ?`, [limite, desde]);
    return rows;
  }

  static async contar() {
    const [rows] = await db.query("SELECT COUNT(*) as total FROM adm_usuarios_jerarquias");
