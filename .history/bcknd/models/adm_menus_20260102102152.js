...
class AdmMenu {
  ...
  static async existsByNombre(nombre, excludeId = null) {
    ...
    if (excludeId) {
      const [rows] = await db.query(
        "SELECT 1 FROM adm_menus WHERE nombre = ? AND id_menu != ? LIMIT 1",
        [nombre, excludeId]
      );
      return Array.isArray(rows) && rows.length > 0;
    }
    const [rows] = await db.query(
      "SELECT 1 FROM adm_menus WHERE nombre = ? LIMIT 1",
      [nombre]
    );
    return Array.isArray(rows) && rows.length > 0;
  }

  static async existsByUrl(url, excludeId = null) {
    if (!url) return false;
    if (excludeId) {
      const [rows] = await db.query(
    ...
    const [rows] = await db.query(
      "SELECT 1 FROM adm_menus WHERE url = ? LIMIT 1",
      [url]
    );
    return Array.isArray(rows) && rows.length > 0;
  }
