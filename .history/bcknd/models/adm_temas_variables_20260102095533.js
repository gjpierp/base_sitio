...
class AdmTemasVariables {
  static async findAll() {
    const [rows] = await db.query(
      "SELECT * FROM adm_temas_variables ORDER BY clave"
    );
    return rows;
  }

  static async listar(desde = 0, limite = 10, sortColumn = "clave", sortDir = "ASC") {
    const allowed = ["id_tema_var", "clave", "etiqueta", "tipo"];
    const col = allowed.includes(sortColumn) ? sortColumn : "clave";
    const dir = String(sortDir).toUpperCase() === "DESC" ? "DESC" : "ASC";
    if (desde === undefined || limite === undefined) {
      const [rows] = await db.query(`SELECT * FROM adm_temas_variables ORDER BY ${col} ${dir}`);
      return rows;
    }
    const [rows] = await db.query(
      `SELECT * FROM adm_temas_variables ORDER BY ${col} ${dir} LIMIT ? OFFSET ?`,
      [limite, desde]
    );
    return rows;
  }
  ...
  static async contar() {
    const [rows] = await db.query("SELECT COUNT(*) as total FROM adm_temas_variables");
    return rows[0]?.total || 0;
  }

  static async findByClave(clave) {
    const [rows] = await db.query(
      "SELECT * FROM adm_temas_variables WHERE clave = ? LIMIT 1",
      [clave]
    );
    return rows[0] || null;
  }
  ...
  static async create(payload) {
    ...
    const vals = [
      payload.clave,
      payload.etiqueta || null,
      payload.tipo || "string",
      payload.valor_defecto || null,
