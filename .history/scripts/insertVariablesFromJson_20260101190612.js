...
try {
  require("dotenv").config({ path: path.join(__dirname, "..", "bcknd", ".env") });
} catch (e) {
  // ignore
}
...
async function main() {
  ...
  let poolModule;
  try {
    poolModule = require(path.join(__dirname, "..", "bcknd", "database", "connections"));
  } catch (err) {
    console.error(
  ...
  try {
    ...
    await conn.commit();
    console.log(
      `OK: ${inserted} registros insertados/actualizados en la tabla 'variables'.`
    );
  } catch (err) {
