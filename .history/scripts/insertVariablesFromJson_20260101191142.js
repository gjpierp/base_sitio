...
async function main() {
  ...
  if (validate) {
    ...
    try {
      ...
      console.log(
        `Validación tabla '${tableName}': columnas reales=${actualCols.length}, payload=${columns.length}`
      );
      if (missing.length)
        console.warn("Columnas faltantes en la tabla:", missing);
  ...
  if (dryRun) {
    ...
    payload.slice(0, 5).forEach((item, idx) => {
      const params = columns.map((k) =>
        item.hasOwnProperty(k) ? item[k] : null
      );
  ...
  try {
    ...
    await conn.commit();
    console.log(
      `OK: ${inserted} registros insertados/actualizados en la tabla '${tableName}'.`
    );
  } catch (err) {
