...
async function run() {
  ...
  try {
    console.log('Candidates:', candidates.map(c=>({ name:c.name, hasPool: !!c.pool}))); 
  } catch(e){}

  const dbName =
    chosen.name ||
    (chosen.pool && chosen.pool.config && chosen.pool.config.connectionConfig && chosen.pool.config.connectionConfig.database) ||
    null;
  if (!dbName) {
    console.error(
