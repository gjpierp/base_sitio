...
async function introspect(pool, dbName) {
  ...
  for (const t of tables) {
    ...
    for (const k of keyRows || []) {
      keys[k.CONSTRAINT_NAME] = (k.cols || '').split(',').map((s) => s.trim()).filter(Boolean);
    }
    ...
    if (changed) fs.writeFileSync(outPath, JSON.stringify(entity, null, 2), 'utf8');
...
async function run() {
  ...
  if (dbAdmin) candidates.push({ pool: dbAdmin, name: process.env.ADMS_DB_NAME });
  try {
  ...
  if (!chosen || !chosen.pool) {
    console.error('No hay una conexión MySQL disponible. Configure VVFF_DB_* o ADMS_DB_* en el entorno.');
    process.exit(1);
  }

  const dbName = chosen.name || (chosen.pool.config && chosen.pool.config.connectionConfig && chosen.pool.config.connectionConfig.database) || null;
  if (!dbName) {
    console.error('No se pudo determinar el nombre de la base de datos. Exporta VVFF_DB_NAME o ADMS_DB_NAME.');
    process.exit(1);
  }
