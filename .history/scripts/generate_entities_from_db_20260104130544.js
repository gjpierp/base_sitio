...
async function run() {
  ...
  // prefer explicit env DB name; prefer ADMS if available
  let chosen = null;
  if (process.env.ADMS_DB_NAME) {
    chosen = candidates.find((c) => c.name === process.env.ADMS_DB_NAME && c.pool) || null;
  }
  if (!chosen) chosen = candidates.find((c) => c.name && c.pool) || null;
