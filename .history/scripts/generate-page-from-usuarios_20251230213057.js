...
const SRC_DIR = path.join(__dirname, '..', 'frntnd', 'src', 'app', 'pages', 'usuarios');
...
async function copyTemplate(targetKebab, singularKebab) {
  if (!targetKebab) {
    console.error('Uso: node scripts/generate-page-from-usuarios.js <target-kebab> [singular-kebab]');
    process.exit(1);
  }
  if (!singularKebab) {
    singularKebab = targetKebab.replace(/s$/, '') || targetKebab;
  }
  const destDir = path.join(__dirname, '..', 'frntnd', 'src', 'app', 'pages', targetKebab);
  ...
  async function processFile(srcPath, relPath) {
    ...
    let out = content
      .split(pluralSrc).join(pluralTarget)
      .split(singularSrc).join(singularTarget)
      .split(PascalPluralSrc).join(PascalPluralTarget)
      .split(PascalSingularSrc).join(PascalSingularTarget);
  ...
  try {
    ...
    console.log('Revisa y ajusta manualmente los nombres y rutas en los ficheros nuevos.');
  } catch (e) {
