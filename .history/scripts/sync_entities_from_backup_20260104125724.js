...
function parseSqlFile(filePath) {
  ...
  const createMatch = txt.match(
    /CREATE TABLE `([^`]+)` \(([\s\S]*?)\) ENGINE=/i
  );
  ...
  for (const line of lines) {
    if (line.startsWith("`")) {
      ...
      if (defMatch)
        col.default = defMatch[1].replace(/^'/, "").replace(/'$/, "");
    ...
    if (
      /CONSTRAINT `([^`]+)` FOREIGN KEY \(([^)]+)\) REFERENCES `([^`]+)` \(([^)]+)\)/i.test(
        line
      )
    ) {
      const m = line.match(
...
function run() {
  ...
  const files = fs
    ...
    .filter((f) => f.startsWith("db_vvff_admin_") && f.endsWith(".sql"));
  const report = [];
