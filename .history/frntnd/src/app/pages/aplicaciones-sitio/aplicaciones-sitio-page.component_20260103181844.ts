  ...
  onEdit(item: any) {
    (async () => {
      try {
        ...
        let row: any = null;
        try {
          const resp: any = await firstValueFrom(this.api.get<any>(`aplicaciones_sitio/${id}`) as any);
        ...
        try {
          const sresp: any = await firstValueFrom(this.api.getPaginated('sitios', { desde: 0 }) as any);
        ...
        formFields.push({ key: 'nombre', label: 'Nombre', type: 'text', value: valOr('nombre', '') });
        ...
        formFields.push({ key: 'descripcion', label: 'Descripción', type: 'text', value: valOr('descripcion', '') });
        formFields.push({ key: 'sitio_id', label: 'Sitio', type: 'select', options: sitioOptions, value: String(valOr('sitio_id', valOr('id_sitio', ''))) });
        formFields.push({ key: 'activo', label: 'Activo', type: 'select', options: [{ value: 'true', label: 'Sí' }, { value: 'false', label: 'No' }], value: String(valOr('activo', true)) });

        const result = await swalForm(`Editar aplicación`, formFields as any, { width: '720px' });
        if (!result) return;
        ...
        try {
          payload.sitio_id = result.sitio_id !== undefined && result.sitio_id !== null && result.sitio_id !== '' ? Number(result.sitio_id) : null;
          if (Number.isNaN(payload.sitio_id)) payload.sitio_id = null;
        } catch {
          payload.sitio_id = null;
        }
        try {
          if (typeof result.activo === 'string') payload.activo = result.activo === 'true' || result.activo === '1';
        ...
        try {
          ...
          await Swal.fire('Error', (err as any)?.error?.msg || 'No se pudo actualizar', 'error');
        }
      } catch (err) {
        try {
          console.error(err);
