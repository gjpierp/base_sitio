...
export class CrearAplicacionesSitioPageComponent implements OnInit {
  ...
  async loadSitios() {
    try {
      const resp: any = await firstValueFrom(this.api.getPaginated<any>('sitios', { desde: 0 }) as any);
      const sitios = Array.isArray(resp?.data) ? resp.data : Array.isArray(resp?.sitios) ? resp.sitios : Array.isArray(resp) ? resp : [];
      const opts = sitios.map((s: any) => ({ label: s.nombre ?? s.title ?? `#${s.id_sitio ?? s.id}`, value: String(s.id_sitio ?? s.id) }));
  ...
  async cargarPorId(id: string) {
    ...
    try {
      ...
      if (row) {
        ...
        try {
          this.modelo.sitio_id = (row.sitio_id ?? row.id_sitio ?? this.modelo.sitio_id ?? '') !== null ? String(row.sitio_id ?? row.id_sitio ?? this.modelo.sitio_id ?? '') : '';
        } catch {}
        try {
          this.modelo.activo = row.activo !== undefined ? String(row.activo) : String(this.modelo.activo);
        } catch {}
      }
  ...
  async guardar(_evt?: any) {
    ...
    try {
      ...
      try {
        if (payload.sitio_id !== undefined && payload.sitio_id !== null && payload.sitio_id !== '') {
      ...
      try {
        if (payload.activo !== undefined && payload.activo !== null) {
          if (typeof payload.activo === 'string') payload.activo = payload.activo === 'true' || payload.activo === '1';
