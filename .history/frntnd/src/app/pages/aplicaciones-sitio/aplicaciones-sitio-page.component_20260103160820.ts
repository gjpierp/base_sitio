  ...
  async load() {
    ...
    try {
      const res: any = await firstValueFrom(
        this.api.getPaginated('aplicaciones_sitio', { desde: 0, limite: 10 }) as any
      );
      const rows = res?.data || [];
  ...
  onPageChange(evt: {
    ...
    if (term) {
      this.api.get<any>(`todo/coleccion/aplicaciones_sitio/${encodeURIComponent(term)}`).subscribe({
        next: (resp) => {
          const list = Array.isArray((resp as any)?.resultados) ? (resp as any).resultados : Array.isArray(resp) ? resp : [];
