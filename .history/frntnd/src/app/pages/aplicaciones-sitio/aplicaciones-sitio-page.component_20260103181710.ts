      ...
      try {
        ...
        if (row) {
          for (const k of Object.keys(row)) {
            ...
            if (Array.isArray(v)) v = `<ul>${v.map((x) => `<li>${JSON.stringify(x)}</li>`).join('')}</ul>`;
            else if (typeof v === 'object') v = `<pre style="white-space:pre-wrap;margin:0">${JSON.stringify(v, null, 2)}</pre>`;
            else v = String(v);
        ...
        const html = fields
          .map((f) => `<div style="margin-bottom:0.5rem"><strong>${f.label}:</strong><div>${f.value}</div></div>`)
          .join('');
