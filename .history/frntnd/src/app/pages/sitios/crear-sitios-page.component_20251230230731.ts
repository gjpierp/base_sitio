...
export class CrearSitiosPageComponent implements OnInit {
  ...
  async cargarPorId(id: string) {
    this.loading = true;
    try {
      const res: any = await firstValueFrom(this.api.get<any>(`sitios/${id}`) as any);
  ...
  async guardar(_evt?: any) {
    ...
    try {
      const id = this.route.snapshot.queryParams['id'];
      if (id) {
        await firstValueFrom(this.api.put(`sitios/${id}`, this.modelo) as any);
        this.notify.info('Sitio actualizado');
      } else {
        await firstValueFrom(this.api.post('sitios', this.modelo) as any);
