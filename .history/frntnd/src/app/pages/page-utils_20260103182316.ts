...
export async function abrirEditarModalGeneric(
  ...
  try {
      ...
      if (ff.options && typeof ff.options === 'object' && ff.options.endpoint) {
        try {
          ...
          ff.options = (Array.isArray(rows) ? rows : []).map((r: any) => ({
            value: String(r[ff.options.valueKey ?? 'id'] ?? r[ff.options.valueKey ?? 'id_' + (ff.options.endpoint?.replace(/s$/,'') )] ?? ''),
            label: r[ff.options.labelKey ?? 'nombre'] ?? String(r[ff.options.valueKey ?? 'id'] ?? ''),
          }));
        } catch {
          ff.options = [];
        }
    ...
    const title = (detail && (detail.nombre || detail.title || detail.titulo)) ? `Editar: ${detail.nombre ?? detail.title ?? detail.titulo}` : `Editar ${endpoint}`;
    ...
    try {
      ...
      try { await (await import('sweetalert2')).default.fire('Guardado', 'Elementos actualizado', 'success'); } catch {}
      // try refresh common load methods
      try { if (typeof thisRef.load === 'function') await thisRef.load(); } catch {}
      try { if (typeof thisRef.refrescar === 'function') await thisRef.refrescar(); } catch {}
      try { if (typeof thisRef.cargarDatosAsync === 'function') await thisRef.cargarDatosAsync(); } catch {}
      return true;
    } catch (err) {
      try { await (await import('sweetalert2')).default.fire('Error', (err as any)?.error?.msg || 'No se pudo actualizar', 'error'); } catch {}
      return false;
    }
  } catch {
    return false;
