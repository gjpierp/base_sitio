          ...
          if (base) {
            ...
            try {
              if (typeof abrirEditarModalGeneric === 'function') {
                ...
                void abrirEditarModalGeneric(thisRef, row, base, [], idKeys).catch(() => {});
                return true;
              }
            } catch {}
          }
  ...
  try {
    ...
    const resolveOptionsForKey = async (key: string) => {
      ...
      for (const cand of candidates) {
        try {
          const resp: any = await firstValueFrom(thisRef.api.getPaginated(cand, { desde: 0 }) as any);
          const rows = resp?.data || resp || [];
          if (Array.isArray(rows) && rows.length > 0) {
            // choose valueKey and labelKey heuristically
            const sample = rows[0] || {};
            const valueKey = ['id', `id_${base}`, Object.keys(sample).find((k) => /id$/i.test(k)) || 'id'][0];
            const labelKey = (['nombre', 'title', 'titulo', 'dominio', 'label'] as string[]).find((k) => k in sample) || Object.keys(sample).find((k) => /name|nombre|title|titulo|label|dominio/i.test(k)) || valueKey;
            return (rows || []).map((r: any) => ({ value: String(r[valueKey] ?? r.id ?? ''), label: String(r[labelKey] ?? r[valueKey] ?? r.id ?? '') }));
          }
        } catch {}
      }
      return [];
      ...
      if ((!fields || fields.length === 0) && detail && typeof detail === 'object') {
        for (const k of Object.keys(detail)) {
          ...
          if (/_id$/.test(k) || /^id_/.test(k)) {
            builtFields.push({ key: k, label: prettify(k), type: 'select', options: { endpointCandidates: buildCandidates(k.replace(/^id_/, '').replace(/_id$/, '')) } });
          } else if (typeof val === 'boolean') {
            builtFields.push({ key: k, label: prettify(k), type: 'select', options: [{ value: 'true', label: 'Sí' }, { value: 'false', label: 'No' }] });
    ...
    for (const f of fields) {
      ...
      if (ff.options && typeof ff.options === 'object' && ff.options.endpoint) {
        try {
          const optResp: any = await firstValueFrom(thisRef.api.getPaginated(ff.options.endpoint, { desde: 0 }) as any);
          ...
          ff.options = (Array.isArray(rows) ? rows : []).map((r: any) => ({
            ...
            label: r[ff.options.labelKey ?? 'nombre'] ?? String(r[ff.options.valueKey ?? 'id'] ?? r.id ?? ''),
          }));
        } catch {
          ff.options = [];
        }
      } else if (ff.options && ff.options.endpointCandidates && Array.isArray(ff.options.endpointCandidates)) {
        // Try candidates in order
        try {
          let resolved: any[] = [];
          for (const cand of ff.options.endpointCandidates) {
            try {
              const optResp: any = await firstValueFrom(thisRef.api.getPaginated(cand, { desde: 0 }) as any);
              const rows = optResp?.data || optResp || [];
              if (Array.isArray(rows) && rows.length > 0) {
                const sample = rows[0] || {};
                const valueKey = (Object.keys(sample).find((k) => /id$/i.test(k)) as string) || 'id';
                const labelKey = (['nombre', 'title', 'titulo', 'dominio', 'label'] as string[]).find((k) => k in sample) || Object.keys(sample).find((k) => /name|nombre|title|titulo|label|dominio/i.test(k)) || valueKey;
                resolved = (rows || []).map((r: any) => ({ value: String(r[valueKey] ?? r.id ?? ''), label: String(r[labelKey] ?? r[valueKey] ?? r.id ?? '') }));
                break;
              }
            } catch {}
          }
