  ...
  try {
    ...
    try {
      ...
      for (const f of fields) {
        ...
        // Required check (only enforced on create)
        if (!thisRef.modalEditingId && (v === '' || v === null || v === undefined)) {
          try {
            const { default: Swal } = await import('sweetalert2');
      ...
      try {
        ...
        console.debug('[onModalConfirmGeneric] submit', endpoint, id ? `update ${id}` : 'create', payload);
      } catch {}
      ...
      if (id) {
        ...
        try {
          console.debug('[onModalConfirmGeneric] update response', res);
        } catch {}
