...
export class CrearThemesAdminPageComponent implements OnInit {
  ...
  async cargarPorId(id: string) {
    this.loading = true;
    try {
      const res: any = await firstValueFrom(this.api.get<any>(`themes-admin/${id}`) as any);
  ...
  async guardar(_evt?: any) {
    ...
    try {
      const id = this.route.snapshot.queryParams['id'];
      if (id) {
        await firstValueFrom(this.api.put(`themes-admin/${id}`, this.modelo) as any);
        this.notify.info('Tema actualizado');
      } else {
        await firstValueFrom(this.api.post('themes-admin', this.modelo) as any);
