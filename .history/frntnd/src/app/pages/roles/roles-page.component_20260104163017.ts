...
@Component({
  selector: 'page-roles',
  standalone: true,
  imports: [CommonModule, UiEntityTableComponent, UiCardComponent, UiSpinnerComponent, UiModalComponent],
  ...
  abrirEditar(row: any) {
    (async () => {
      try {
        ...
        // fetch full detail before opening modal so it opens with all data on first click
        let payload: any = row || {};
        if (id) {
          try {
            ...
            payload = resp?.data ?? resp ?? payload;
          } catch {}
        }
        ...
        // open modal after data prepared
        this.modalOpen = true;
        try {
          this.cdr.detectChanges();
        } catch {}
