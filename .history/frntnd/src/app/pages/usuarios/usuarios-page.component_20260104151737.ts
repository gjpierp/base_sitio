  ...
  async openEditModal(usuario: any) {
    try {
      ...
      try {
        ...
        if (idToFetch) {
          const detailResp: any = await firstValueFrom(this.api.get(`usuarios/${idToFetch}`) as any).catch(
            () => null
          );
          if (detailResp) {
      ...
      (async () => {
        try {
          ...
          for (const f of this.modalFields) {
            ...
            this.modalValues[f.key] = (current !== undefined && current !== null && String(current) !== '')
              ? current
