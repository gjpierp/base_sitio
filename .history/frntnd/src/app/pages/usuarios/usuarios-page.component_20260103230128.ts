...
export class UsuariosPageComponent implements OnInit {
  ...
  async openEditModal(usuario: any) {
    try {
      ...
      // Si el objeto usuario no contiene campos completos (p.ej. id_rol, id_jerarquia), intentar cargarlo desde API
      try {
        const uid = usuario?.id || usuario?.id_usuario || usuario?.ID || null;
        if (uid && (!usuario?.id_rol || !usuario?.id_jerarquia)) {
          try {
            const resp: any = await firstValueFrom(this.api.get(`usuarios/${uid}`) as any);
            if (resp && resp.usuario) {
              usuario = resp.usuario;
            }
          } catch (e) {
            // no bloquear, continuar con lo que tengamos
            console.debug('[openEditModal] no se pudo obtener usuario completo', e);
          }
        }
      } catch (e) {
        try {
