...
export class UsuariosPageComponent implements OnInit {
   ...
   * @date 28-12-2025
   */
  private buildUsuarioFields(opts: any = {}, defaults: any = {}) {
    ...
    // Build fields from model `usuario.ts` excluding sensitive/auto fields
    // Excluded: id_usuario, hash_contrasena, fecha_cambio_contrasena, fecha_creacion, fecha_actualizacion
    const fields: any[] = [];

    // optional readonly front-end id
    fields.push({ key: 'id', label: 'ID', type: 'text', value: defaults.id ?? '', readonly: true, hidden: false });

    // Basic identity
    fields.push({ key: 'nombre_usuario', label: 'Nombre de usuario', type: 'text', value: defaults.nombre_usuario ?? defaults.nombre ?? '' });
    fields.push({ key: 'nombres', label: 'Nombres', type: 'text', value: defaults.nombres ?? '' });
    fields.push({ key: 'apellidos', label: 'Apellidos', type: 'text', value: defaults.apellidos ?? '' });

    // Contact
    fields.push({ key: 'correo_electronico', label: 'Correo electrónico', type: 'text', value: defaults.correo_electronico ?? defaults.correo ?? '' });

    // Imagen / avatar
    fields.push({ key: 'img', label: 'Avatar (URL)', type: 'text', value: defaults.img ?? '' });

    // Configuración y relaciones
    fields.push({ key: 'id_configuracion_default', label: 'Configuración por defecto', type: 'text', value: defaults.id_configuracion_default ?? '' });

    // UI-related relation fields (selects)
    fields.push({ key: 'id_rol', label: 'Rol', type: 'select', options: rolOptions, value: defaults.id_rol ?? defaults.idRol ?? rolOptions[0]?.value ?? '' });
    fields.push({ key: 'id_jerarquia', label: 'Jerarquía', type: 'select', options: jerarquiaOptions, value: defaults.id_jerarquia ?? defaults.idJerarquia ?? jerarquiaOptions[0]?.value ?? '' });
    fields.push({ key: 'id_tipo_usuario', label: 'Tipo usuario', type: 'select', options: tipoOptions, value: defaults.id_tipo_usuario ?? tipoOptions[0]?.value ?? '' });
    fields.push({ key: 'id_estado', label: 'Estado', type: 'select', options: estadoOptions, value: defaults.id_estado ?? estadoOptions[0]?.value ?? '' });

    // Active flag
    fields.push({ key: 'activo', label: 'Activo', type: 'select', options: [ { value: 'true', label: 'Sí' }, { value: 'false', label: 'No' } ], value: typeof defaults.activo !== 'undefined' ? (defaults.activo ? 'true' : 'false') : 'true' });
