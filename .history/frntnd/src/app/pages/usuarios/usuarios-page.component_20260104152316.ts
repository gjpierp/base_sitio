...
export class UsuariosPageComponent implements OnInit {
  ...
  async openEditModal(usuario: any) {
    try {
      ...
      console.log('[UsuariosPage] initial modalFields built', this.modalFields.map((f:any)=>({key:f.key,value:f.value,readonly:f.readonly,hidden:f.hidden}))); 
      this.modalValues = {};
      for (const f of this.modalFields) this.modalValues[f.key] = f.value ?? '';
      ...
      (async () => {
        try {
          ...
          console.log('[UsuariosPage] modalFields after options', this.modalFields.map((f:any)=>({key:f.key,optionsLength:(f.options||[]).length}))); 
          try {
            this.cdr.detectChanges();
          } catch {}
        } catch (e) {
