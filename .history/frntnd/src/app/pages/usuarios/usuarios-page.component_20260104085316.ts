      ...
      (async () => {
        try {
          ...
          const rolOptions = (Array.isArray(rrows) ? rrows : []).map((r: any) => ({
            ...
            label:
              r.nombre ?? r.label ?? r.title ?? r.descripcion ?? r.nombre_rol ?? r.rol ?? r.name ?? String(r.id_rol ?? r.id ?? ''),
          }));
          const jerarquiaOptions = (Array.isArray(jrows) ? jrows : []).map((j: any) => ({
            value: String(j.id_jerarquia ?? j.id ?? ''),
            label:
              j.nombre ?? j.label ?? j.title ?? j.descripcion ?? j.nombre_jerarquia ?? j.jerarquia ?? j.name ?? String(j.id_jerarquia ?? j.id ?? ''),
          }));
