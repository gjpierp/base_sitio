...
export class UsuariosPageComponent implements OnInit {
  ...
  async openEditModal(usuario: any) {
    try {
      ...
      this.modalTitle = 'Editar usuario';
      this.modalDeleteMode = false;
      // Fetch full user detail and option lists, then open modal with data (single click)
      try {
        // fetch detail
        try {
        ...
        // fetch option lists in parallel
        const [rresp, jresp, tresp, eresp] = await Promise.all([
          firstValueFrom(this.api.getPaginated('roles', { desde: 0 }) as any).catch(
            () => ({ data: [] } as any)
          ),
        ...
        const rolOptions = (Array.isArray(rrows) ? rrows : []).map((r: any) => ({
          value: String(r.id_rol ?? r.id ?? ''),
          label:
            r.nombre ?? r.label ?? r.title ?? r.descripcion ?? r.nombre_rol ?? r.rol ?? r.name ??
            String(r.id_rol ?? r.id ?? ''),
        }));
        const jerarquiaOptions = (Array.isArray(jrows) ? jrows : []).map((j: any) => ({
          value: String(j.id_jerarquia ?? j.id ?? ''),
          label:
            j.nombre ?? j.label ?? j.title ?? j.descripcion ?? j.nombre_jerarquia ?? j.jerarquia ??
            j.name ?? String(j.id_jerarquia ?? j.id ?? ''),
        }));
        const tipoOptions = (Array.isArray(trows) ? trows : []).map((t: any) => ({
          value: String(t.id_tipo_usuario ?? t.id ?? ''),
        ...
        // fetch user's assigned roles/jerarquias
        try {
          ...
          if (uid) {
            ...
            try {
              const uj =
                userJerqsResp?.usuarios_jerarquias ?? userJerqsResp?.data ?? userJerqsResp ?? null;
