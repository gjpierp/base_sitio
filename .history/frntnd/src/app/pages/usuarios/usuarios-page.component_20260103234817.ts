...
export class UsuariosPageComponent implements OnInit {
  ...
  async openCreateModal() {
    try {
      ...
      // Abrir modal después de preparar campos y valores (usar setTimeout para evitar
      // que el click que abrió el modal se propague al backdrop recién creado)
      setTimeout(() => {
        this.modalOpen = true;
        try {
  ...
  onRemove(usuario: any) {
    ...
    try {
      ...
      // abrir con setTimeout para evitar cierre inmediato por propagación del click
      setTimeout(() => {
        this.modalOpen = true;
        try {
          this.cdr.detectChanges();
  ...
  async openEditModal(usuario: any) {
    try {
      ...
      const fields: any[] = [];
      fields.push({ key: 'nombre_usuario', label: 'Nombre de usuario', type: 'text', value: u?.nombre_usuario ?? u?.nombre ?? '' });
      fields.push({ key: 'correo_electronico', label: 'Correo electrónico', type: 'text', value: u?.correo_electronico ?? u?.correo ?? '' });
      fields.push({ key: 'id_rol', label: 'Rol', type: 'select', options: [], value: String(u?.id_rol ?? '') });
      fields.push({ key: 'id_jerarquia', label: 'Jerarquía', type: 'select', options: [], value: String(u?.id_jerarquia ?? '') });
      fields.push({ key: 'id_tipo_usuario', label: 'Tipo usuario', type: 'select', options: [], value: String(u?.id_tipo_usuario ?? '') });
      fields.push({ key: 'id_estado', label: 'Estado', type: 'select', options: [], value: String(u?.id_estado ?? '') });
      fields.push({ key: 'activo', label: 'Activo', type: 'select', options: [ { value: 'true', label: 'Sí' }, { value: 'false', label: 'No' } ], value: u?.activo ? 'true' : 'false' });
      ...
      this.modalTitle = 'Editar usuario';
      this.modalDeleteMode = false;
      // open asynchronously to prevent click propagation closing the backdrop
      setTimeout(() => {
        this.modalOpen = true;
      ...
      (async () => {
        try {
          const [rresp, jresp, tresp, eres] = await Promise.all([
            firstValueFrom(this.api.getPaginated('roles', { desde: 0 }) as any).catch(() => ({ data: [] })),
            firstValueFrom(this.api.getPaginated('jerarquias', { desde: 0 }) as any).catch(() => ({ data: [] })),
            firstValueFrom(this.api.getPaginated('tipos_usuario', { desde: 0 }) as any).catch(() => ({ data: [] })),
            firstValueFrom(this.api.getPaginated('estados', { desde: 0 }) as any).catch(() => ({ data: [] })),
          ]);
          const rrows = rresp?.data || rresp || [];
          const jrows = jresp?.data || jresp || [];
          const trows = tresp?.data || tresp || [];
          ...
          // Map options
          const rolOptions = (Array.isArray(rrows) ? rrows : []).map((r: any) => ({ value: String(r.id_rol ?? r.id ?? ''), label: r.nombre ?? String(r.id_rol ?? r.id ?? '') }));
          const jerarquiaOptions = (Array.isArray(jrows) ? jrows : []).map((j: any) => ({ value: String(j.id_jerarquia ?? j.id ?? ''), label: j.nombre ?? String(j.id_jerarquia ?? j.id ?? '') }));
          const tipoOptions = (Array.isArray(trows) ? trows : []).map((t: any) => ({ value: String(t.id_tipo_usuario ?? t.id ?? ''), label: t.nombre ?? String(t.id_tipo_usuario ?? t.id ?? '') }));
          const estadoOptions = (Array.isArray(erows) ? erows : []).map((e: any) => ({ value: String(e.id_estado ?? e.id ?? ''), label: e.nombre ?? String(e.id_estado ?? e.id ?? '') }));
          ...
          // If user has no id_rol/id_jerarquia, try to fetch assigned ones
          const uid = u?.id || u?.id_usuario || u?.ID || null;
          if (uid) {
            try {
              const rolesResp: any = await firstValueFrom(this.api.get(`usuarios/roles/${uid}`) as any).catch(() => ({}));
              const assignedRoles = rolesResp?.roles || [];
              if ((!u?.id_rol || String(u.id_rol) === '') && Array.isArray(assignedRoles) && assignedRoles.length) {
                this.modalValues['id_rol'] = String(assignedRoles[0].id_rol ?? assignedRoles[0].id ?? assignedRoles[0].id_rol);
              }
            } catch (e) { console.debug('[openEditModal] roles fetch error', e); }

            try {
              const ujResp: any = await firstValueFrom(this.api.getPaginated(`usuarios_jerarquias`, { desde: 0, id_usuario: uid }) as any).catch(() => ({ data: [] }));
              const asociaciones = ujResp?.data || ujResp?.usuarios_jerarquias || ujResp || [];
              if ((!u?.id_jerarquia || String(u.id_jerarquia) === '') && Array.isArray(asociaciones) && asociaciones.length) {
                this.modalValues['id_jerarquia'] = String(asociaciones[0].id_jerarquia ?? asociaciones[0].id ?? '');
              }
            } catch (e) { console.debug('[openEditModal] usuarios_jerarquias fetch error', e); }
          }
