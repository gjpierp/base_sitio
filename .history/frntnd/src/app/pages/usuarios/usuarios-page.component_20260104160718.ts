...
export class UsuariosPageComponent implements OnInit {
  ...
  async openEditModal(usuario: any) {
    try {
      ...
      this.modalTitle = 'Editar usuario';
      this.modalDeleteMode = false;
      // Open modal immediately so the user sees it on first click.
      try {
        console.log(
          '[UsuariosPage] opening modal immediate modalFields.len=',
          this.modalFields?.length
        );
        this.modalLoading = true;
        this.modalOpen = true;
        // If we have a child reference, call the public helper to ensure the setter runs
        try {
        ...
        console.debug('[UsuariosPage] opening modal error', e);
      }
      (async () => {
        try {
