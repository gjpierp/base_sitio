...
export class UsuariosPageComponent implements OnInit {
  ...
  async openCreateModal() {
    try {
      ...
      fields.push({ key: 'correo_electronico', label: 'Correo electrónico', type: 'text', value: '' });
      fields.push({ key: 'id_rol', label: 'Rol', type: 'select', options: rolOptions, value: rolOptions[0]?.value ?? '' });
      fields.push({ key: 'id_jerarquia', label: 'Jerarquía', type: 'select', options: jerarquiaOptions, value: jerarquiaOptions[0]?.value ?? '' });
      fields.push({ key: 'id_tipo_usuario', label: 'Tipo usuario', type: 'select', options: tipoOptions, value: tipoOptions[0]?.value ?? '' });
      fields.push({ key: 'id_estado', label: 'Estado', type: 'select', options: estadoOptions, value: estadoOptions[0]?.value ?? '' });
      fields.push({ key: 'activo', label: 'Activo', type: 'select', options: [ { value: 'true', label: 'Sí' }, { value: 'false', label: 'No' } ], value: 'true' });
