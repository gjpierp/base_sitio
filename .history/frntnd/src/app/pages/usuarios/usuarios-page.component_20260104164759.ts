  ...
  ngOnInit() {
    ...
    try {
      setupList(this, 'usuarios', 'usuarios', (r: any) => ({
        ...
        nombre:
          (r.nombre_usuario ?? r.nombre ?? r.name ?? '') +
          (r.apellido || r.apellidos ? ' ' + (r.apellido ?? r.apellidos) : ''),
        ...
        try {
          ...
          if (uid) {
            const [userRolesResp, userJerqsResp]: any = await Promise.all([
              ...
              firstValueFrom(this.api.get('usuarios_jerarquias', { id_usuario: uid }) as any).catch(
                () => null
              ),
            ]);
            try {
