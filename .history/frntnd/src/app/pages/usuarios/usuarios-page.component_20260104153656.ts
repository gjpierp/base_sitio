  ...
  async onEdit(usuario: any) {
    try {
      ...
      try {
        ...
        this.cdr.detectChanges();
        try {
          console.log('[UsuariosPage] modalOpen value after set ->', this.modalOpen);
          console.log('[UsuariosPage] modalComp (ViewChild) ->', this.modalComp);
          console.log('[UsuariosPage] modalComp.open ->', this.modalComp?.open);
          // Force the child component's input in case binding didn't propagate yet
          try {
            if (this.modalComp) {
              this.modalComp.open = true;
              console.log('[UsuariosPage] forced modalComp.open ->', this.modalComp.open);
              this.cdr.detectChanges();
            }
          } catch (e) {}
        } catch {}
      ...
      (async () => {
        try {
          ...
          try {
            ...
            if (uid) {
              const [userRolesResp, userJerqsResp] = await Promise.all([
                (this.api.get(`usuarios/roles/${uid}`) as any).toPromise?.() ?? firstValueFrom(this.api.get(`usuarios/roles/${uid}`) as any).catch(() => null),
                firstValueFrom(this.api.get('usuarios_jerarquias', { id_usuario: uid }) as any).catch(() => null),
              ]);
              try {
              ...
              try {
                const uj = userJerqsResp?.usuarios_jerarquias ?? userJerqsResp?.data ?? userJerqsResp ?? null;
