...
export class UsuariosPageComponent implements OnInit {
  ...
  private buildUsuarioFields(opts: any = {}, defaults: any = {}, isEdit: boolean = false) {
    ...
    const fields: any[] = schemaFields.map((s: any) => {
      ...
      base.hidden = base.hidden || (!isEdit && !!base.hiddenOnCreate) || (isEdit && !!base.hiddenOnEdit);
      ...
      // common fallbacks
      candidates.push(key.replace(/_/g, ''), key.replace(/_/g, ' '), 'nombre', 'name', 'correo', 'correo_electronico');

      let val: any = undefined;
      for (const k of candidates) {
        if (!k) continue;
        if (Object.prototype.hasOwnProperty.call(defaults, k) && typeof defaults[k] !== 'undefined') {
          val = defaults[k];
          break;
        }
      }
