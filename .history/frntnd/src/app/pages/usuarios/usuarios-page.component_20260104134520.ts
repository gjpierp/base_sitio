...
export class UsuariosPageComponent implements OnInit {
  ...
  private buildUsuarioFields(opts: any = {}, defaults: any = {}) {
    ...
    const estadoOptions = opts.estadoOptions ?? [];
      const schemaFields = (USUARIO_SCHEMA && Array.isArray(USUARIO_SCHEMA.fields)) ? USUARIO_SCHEMA.fields : [];
      const fields: any[] = schemaFields.map((s: any) => {
        ...
        if (base.type === 'select') {
          ...
          else if (key === 'activo') base.options = [ { value: 'true', label: 'Sí' }, { value: 'false', label: 'No' } ];
        }
        // value: prefer defaults, then alias, then empty
        const aliasKeys = [key, (s.alias || null)];
    ...
    fields.push({
      ...
      type: 'text',
      value: defaults.nombre_usuario ?? defaults.nombre ?? '',
    });
    ...
    fields.push({
      ...
      type: 'text',
      value: defaults.correo_electronico ?? defaults.correo ?? '',
    });
    ...
    fields.push({
      ...
      type: 'text',
      value: defaults.id_configuracion_default ?? '',
    });
    ...
    fields.push({
      ...
      type: 'select',
      options: rolOptions,
      value: defaults.id_rol ?? defaults.idRol ?? rolOptions[0]?.value ?? '',
    });
    fields.push({
      key: 'id_jerarquia',
      label: 'Jerarquía',
      ...
      value: defaults.id_jerarquia ?? defaults.idJerarquia ?? jerarquiaOptions[0]?.value ?? '',
    });
    fields.push({
      key: 'id_tipo_usuario',
      label: 'Tipo usuario',
      type: 'select',
      options: tipoOptions,
      value: defaults.id_tipo_usuario ?? tipoOptions[0]?.value ?? '',
    });
    fields.push({
      key: 'id_estado',
      label: 'Estado',
      ...
      value: defaults.id_estado ?? estadoOptions[0]?.value ?? '',
    });
    ...
    fields.push({
      ...
      value: typeof defaults.activo !== 'undefined' ? (defaults.activo ? 'true' : 'false') : 'true',
    });
