...
export class UsuariosPageComponent implements OnInit {
  ...
  async onEdit(usuario: any) {
    try {
      ...
      // Abrir modal y cargar detalles/fields (openEditModal gestiona el estado)
  ...
  async openEditModal(usuario: any) {
    try {
      // Guard: si ya hay un modal abierto o en carga, ignorar llamadas repetidas
      if (this.modalOpen || this.modalLoading) {
        try {
          console.debug('[UsuariosPage] openEditModal ignored: modal already open/loading');
        } catch {}
        return;
      }
      ...
      // We'll fetch full user details in background so the modal can open immediately
      ...
      this.modalTitle = 'Editar usuario';
      this.modalDeleteMode = false;
      // Start background loading; modal will open automatically when all data is ready
      try {
        console.log(
          '[UsuariosPage] starting background load before opening modal, modalFields.len=',
          this.modalFields?.length
        );
        this.modalLoading = true;
      } catch (e) {
        console.debug('[UsuariosPage] start background load error', e);
      }
      (async () => {
        try {
          // Fetch full user detail now (background)
          try {
            ...
            console.debug('[openEditModal] could not fetch user detail (bg)', e);
          }
          const [rresp, jresp, tresp, eresp] = await Promise.all([
            firstValueFrom(this.api.getPaginated('roles', { desde: 0 }) as any).catch(
              () =>
          ...
          // Additionally, fetch the user's assigned roles and jerarquias to pre-select options
          try {
            const uid = this.modalEditingId || u?.id || u?.id_usuario || null;
            if (uid) {
              const [userRolesResp, userJerqsResp]: any = await Promise.all([
                (this.api.get(`usuarios/roles/${uid}`) as any).toPromise?.() ??
                  firstValueFrom(this.api.get(`usuarios/roles/${uid}`) as any).catch(() => null),
                firstValueFrom(
                  this.api.get('usuarios_jerarquias', { id_usuario: uid }) as any
                ).catch(() => null),
              ] as any);
              try {
                const ur = userRolesResp?.roles ?? userRolesResp?.data ?? userRolesResp ?? null;
                if (Array.isArray(ur) && ur.length) {
                  const first = ur[0];
                  const val = first?.id_rol ?? first?.id ?? null;
                  if (val != null) this.modalValues['id_rol'] = String(val);
                }
              } catch (e) {}
              try {
                const uj =
                  userJerqsResp?.usuarios_jerarquias ??
                  userJerqsResp?.data ??
                  userJerqsResp ??
                  null;
                if (Array.isArray(uj) && uj.length) {
                  const first = uj[0];
                  const val = first?.id_jerarquia ?? first?.id ?? null;
                  if (val != null) this.modalValues['id_jerarquia'] = String(val);
                }
              } catch (e) {}
            }
          } catch (e) {
            console.debug('[openEditModal] could not fetch user roles/jerarquias', e);
          }
          ...
          // Attempt to infer select values (rol/jerarquia/otros) from user payload when empty
          try {
            for (const f of this.modalFields) {
              if (String(f.type).toLowerCase() !== 'select') continue;
              const key = f.key;
              const cur = this.modalValues[key];
              if (cur !== undefined && cur !== null && String(cur) !== '') continue;
              const base = String(key).replace(/^id_/, '');
              const candidates: any[] = [];
              // direct properties
              candidates.push(u?.[key]);
              candidates.push(u?.[base]);
              candidates.push(u?.[`${base}_nombre`]);
              candidates.push(u?.[`${base}_name`]);
              candidates.push(u?.[`nombre_${base}`]);
              candidates.push(u?.['nombre']);
              candidates.push(u?.['name']);
              // arrays of relations (roles, jerarquias)
              if (Array.isArray(u?.roles) && u.roles.length) {
                const r0 = u.roles[0];
                candidates.push(r0?.id_rol ?? r0?.id ?? r0?.[base]);
                candidates.push(r0?.nombre ?? r0?.name);
              }
              if (Array.isArray(u?.jerarquias) && u.jerarquias.length) {
                const j0 = u.jerarquias[0];
                candidates.push(j0?.id_jerarquia ?? j0?.id ?? j0?.[base]);
                candidates.push(j0?.nombre ?? j0?.name);
              }
              // try to match any candidate against options
              const opts = f.options || [];
              let matched: any = null;
              for (const c of candidates) {
                if (c === undefined || c === null) continue;
                const cs = String(c);
                matched = opts.find((o: any) => String(o.value) === cs);
                if (matched) break;
                matched = opts.find((o: any) => String(o.label).toLowerCase() === cs.toLowerCase());
                if (matched) break;
                matched = opts.find((o: any) =>
                  String(o.label).toLowerCase().includes(cs.toLowerCase())
                );
                if (matched) break;
              }
              if (matched) {
                this.modalValues[key] = String(matched.value);
              }
            }
          } catch (e) {
            console.debug('[openEditModal] infer select value error', e);
          }
          console.log(
          ...
          console.log('[UsuariosPage] modalValues after options', this.modalValues);
          // All background loads finished: set loading false and open modal
          try {
            this.modalLoading = false;
          } catch {}
          try {
            // open modal in next macrotask to avoid change-detection errors
            setTimeout(() => {
              try {
                this.modalOpen = true;
                if (this.modalComp && typeof this.modalComp.openModal === 'function') {
                ...
                console.debug('[UsuariosPage] open modal after load error', e);
              }
            }, 0);
          } catch {}
        } catch (e) {
