...
export class UsuariosPageComponent implements OnInit {
  ...
  async openCreateModal() {
    try {
      ...
      fields.push({ key: 'contrasena', label: 'Contraseña', type: 'password', value: '' });
      fields.push({ key: 'nombres', label: 'Nombres', type: 'text', value: '' });
      fields.push({ key: 'apellidos', label: 'Apellidos', type: 'text', value: '' });
      fields.push({
  ...
  async openEditModal(usuario: any) {
    try {
      ...
      try {
        const rresp: any = await firstValueFrom(this.api.getPaginated('roles', { desde: 0 }) as any);
        const rrows = rresp?.data || rresp || [];
        rolOptions = (Array.isArray(rrows) ? rrows : []).map((r: any) => ({ value: String(r.id_rol ?? r.id ?? ''), label: r.nombre ?? String(r.id_rol ?? r.id ?? '') }));
      } catch {}
      try {
        const jresp: any = await firstValueFrom(this.api.getPaginated('jerarquias', { desde: 0 }) as any);
        const jrows = jresp?.data || jresp || [];
        jerarquiaOptions = (Array.isArray(jrows) ? jrows : []).map((j: any) => ({ value: String(j.id_jerarquia ?? j.id ?? ''), label: j.nombre ?? String(j.id_jerarquia ?? j.id ?? '') }));
      } catch {}
      try {
        const tresp: any = await firstValueFrom(this.api.getPaginated('tipos_usuario', { desde: 0 }) as any);
        const trows = tresp?.data || tresp || [];
        tipoOptions = (Array.isArray(trows) ? trows : []).map((t: any) => ({ value: String(t.id_tipo_usuario ?? t.id ?? ''), label: t.nombre ?? String(t.id_tipo_usuario ?? t.id ?? '') }));
      } catch {}
      try {
        const eres: any = await firstValueFrom(this.api.getPaginated('estados', { desde: 0 }) as any);
        const erows = eres?.data || eres || [];
        estadoOptions = (Array.isArray(erows) ? erows : []).map((e: any) => ({ value: String(e.id_estado ?? e.id ?? ''), label: e.nombre ?? String(e.id_estado ?? e.id ?? '') }));
      } catch {}

      const fields: any[] = [];
      fields.push({ key: 'nombre_usuario', label: 'Nombre de usuario', type: 'text', value: usuario?.nombre_usuario ?? usuario?.nombre ?? '' });
      // contraseña en blanco para edición (opcional)
      fields.push({ key: 'contrasena', label: 'Contraseña', type: 'password', value: '' });
      fields.push({ key: 'nombres', label: 'Nombres', type: 'text', value: usuario?.nombres ?? '' });
      fields.push({ key: 'apellidos', label: 'Apellidos', type: 'text', value: usuario?.apellidos ?? '' });
      fields.push({ key: 'correo_electronico', label: 'Correo electrónico', type: 'text', value: usuario?.correo_electronico ?? usuario?.correo ?? '' });
      fields.push({ key: 'id_rol', label: 'Rol', type: 'select', options: rolOptions, value: String(usuario?.id_rol ?? '') });
      fields.push({ key: 'id_jerarquia', label: 'Jerarquía', type: 'select', options: jerarquiaOptions, value: String(usuario?.id_jerarquia ?? '') });
      fields.push({ key: 'id_tipo_usuario', label: 'Tipo usuario', type: 'select', options: tipoOptions, value: String(usuario?.id_tipo_usuario ?? '') });
      fields.push({ key: 'id_estado', label: 'Estado', type: 'select', options: estadoOptions, value: String(usuario?.id_estado ?? '') });
      fields.push({ key: 'activo', label: 'Activo', type: 'select', options: [ { value: 'true', label: 'Sí' }, { value: 'false', label: 'No' } ], value: usuario?.activo ? 'true' : 'false' });
