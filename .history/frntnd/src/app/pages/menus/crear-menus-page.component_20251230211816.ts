  ...
  private async cargarPadresAsync() {
    try {
      ...
      try {
        ...
        console.error('[CrearMenus] error applying padre options', e);
      }
    } catch (err: any) {
      console.error('[CrearMenus] cargarPadresAsync error', err);
      // Mostrar mensaje de error en la UI
      try {
        this.errorMsg = 'No se pudieron cargar los menús padres';
        this.cdr.detectChanges();
      } catch (e) {}
      // Si la causa es falta de autentificación, redirigir al login
      if (err && (err.status === 401 || (err.error && err.error.msg && err.error.msg.toLowerCase().includes('token')))) {
        try {
          this.router.navigate(['/login'], { queryParams: { redirectTo: this.router.url } });
        } catch (e) {
          console.error('[CrearMenus] redirect to login failed', e);
        }
      }
    }
  }
