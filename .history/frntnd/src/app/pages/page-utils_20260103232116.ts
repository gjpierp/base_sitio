...
export async function onModalConfirmGeneric(thisRef: any, endpoint: string) {
  try {
    ...
    try {
      ...
      try {
        // For usuarios endpoint, ensure backend-required fields exist
        if (String(endpoint).toLowerCase() === 'usuarios') {
          try {
            // Guarantee password exists and meets length requirements
            if (!payload.contrasena) {
              const rand = Math.random().toString(36).slice(2, 8);
              payload.contrasena = `Pwd${rand}`; // >=6 chars
            }
            // Derive nombres/apellidos from nombre_usuario if possible
            const nombreUsuario = payload.nombre_usuario || payload.nombre || '';
            if (!payload.nombres) {
              const parts = String(nombreUsuario || '').trim().split(/\s+/).filter(Boolean);
              payload.nombres = parts.length ? parts[0] : (payload.correo_electronico || 'Usuario').split('@')[0];
            }
            ...
            try {
              console.debug('[onModalConfirmGeneric] error setting usuario defaults', e);
            } catch {}
          }
        }
