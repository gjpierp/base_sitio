...
export function onPageChangeGeneric(
  thisRef: any,
  evt: { page: number; pageSize: number; term?: string; sortKey?: string; sortDir?: 'asc' | 'desc' },
  endpoint: string,
  mapFn: (r: any) => any
) {
  ...
  if (term) {
    thisRef.api.get<any>(`todo/coleccion/${endpoint}/${encodeURIComponent(term)}`).subscribe({
      next: (resp: any) => {
        const list = Array.isArray(resp?.resultados) ? resp.resultados : Array.isArray(resp) ? resp : [];
...
export function abrirEditarGeneric(thisRef: any, row: any, routePath: string, idKeys: string[] = ['id', 'id_menu', 'ID']) {
  try {
    let id: any = null;
