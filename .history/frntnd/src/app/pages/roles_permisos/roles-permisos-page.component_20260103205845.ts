...
export class RolesPermisosPageComponent implements OnInit {
  ...
  onPageChange(evt: { page: number; pageSize: number; term?: string; sortKey?: string; sortDir?: 'asc' | 'desc' }) {
    this.currentPage = evt.page;
    // simple server-side pagination not implemented for this endpoint; keep stub
    // but ensure UI reflects the selected page
    try {
      this.cdr.detectChanges();
    } catch {}
  }
