...
import { setupList, onPageChangeGeneric, abrirEditarGeneric, abrirEditarModalGeneric } from '../page-utils';
...
export class TiposUsuariosPageComponent implements OnInit {
  ...
  ngOnInit() {
    setupList(this, 'tipos-usuarios', 'tipos_usuarios', (r: any) => ({
      id_tipo_usuario: r.id_tipo_usuario ?? r.id ?? r.ID,
      nombre: r.nombre ?? '',
  ...
  onPageChange(evt: {
    ...
    onPageChangeGeneric(this, evt, 'tipos-usuarios', (r: any) => ({
      id_tipo_usuario: r.id_tipo_usuario ?? r.id ?? r.ID,
      nombre: r.nombre ?? '',
      descripcion: r.descripcion ?? '',
    }));
