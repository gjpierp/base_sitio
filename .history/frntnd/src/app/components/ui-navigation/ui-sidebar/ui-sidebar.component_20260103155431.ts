...
export class UiSidebarComponent implements OnInit, OnDestroy {
  ...
  private syncLoginAndMaybeLoad() {
    ...
    // Si el backend falla o devuelve vacío, usar un menú por defecto en `assets/menu-default.json`.
    this.loading = true;
    this.api.get<{ ok: boolean; menus: any[] }>('menus/arbol').subscribe({
      next: (resp) => {
        const arbol = Array.isArray(resp?.menus) ? resp.menus : [];
        if (!arbol || !arbol.length) {
          ...
          fetch('/assets/menu-default.json')
            .then((r) => r.json())
            .then((d) => {
              const fallback = Array.isArray(d) ? d : [];
              this.menuItems = this.sanitizeMenuTree(fallback);
              localStorage.setItem('menu', JSON.stringify(this.menuItems));
              this.loading = false;
              this.autoExpandForCurrentRoute();
            })
      ...
      error: () => {
        // Backend error: usar fallback local
        fetch('/assets/menu-default.json')
          .then((r) => r.json())
          .then((d) => {
            const fallback = Array.isArray(d) ? d : [];
  ...
  private sanitizeMenuTree(
    ...
    const walk = (list: any[], parentPrefix: string | null): Array<any> => {
      const out: any[] = [];
      // Ordenar segun campo 'orden' si lo provee el backend (por defecto 0)
      // Normalizar campo 'orden' a número y usar un tie-breaker por nombre
      const sorted = (list || []).slice().sort((a: any, b: any) => {
        const rawA = a && (a.orden ?? a['orden']);
        const rawB = b && (b.orden ?? b['orden']);
        const oa = Number(rawA ?? 0) || 0;
        const ob = Number(rawB ?? 0) || 0;
        if (oa !== ob) return oa < ob ? -1 : 1;
        // Si los valores de orden coinciden, desempatar por nombre (siempre texto)
        const nameA = (
